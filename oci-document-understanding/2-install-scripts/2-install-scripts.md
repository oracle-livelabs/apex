# Setup Tasks for Application Deployment
...
1. Return to your APEX Workspace and click **'⌄'** next to **SQL Workshop** tab, then click **SQL Scripts**.

   ![Select Quick SQL](images/sql_script.png " ")

2. Copy the following SQL code and paste it into the code editor:
    ...
    INSERT INTO EMPLOYEE_DETAILS (EMPNO, EMP_NAME, MGR)
    VALUES
  (10, 'JOHN', 20);
INSERT INTO EMPLOYEE_DETAILS (EMPNO, EMP_NAME, MGR)
VALUES
...
/ CREATE TABLE "INV_UPLOAD" (
  ...
  "DOC_AI_JSON" CLOB,
  CONSTRAINT "INV_UPLOAD_ID_PK" PRIMARY KEY ("ID") USING INDEX ENABLE
);
CREATE
OR REPLACE EDITIONABLE TRIGGER "INV_UPLOAD_BIU" BEFORE INSERT
OR
UPDATE
  ON INV_UPLOAD FOR EACH ROW BEGIN IF INSERTING THEN : NEW.CREATED := SYSDATE;
...
/ CREATE TABLE "DOCAI_RESPONSE" (
  ...
  "UPDATED" DATE NOT NULL ENABLE,
  "UPDATED_BY" VARCHAR2(255 CHAR) NOT NULL ENABLE,
  CONSTRAINT "DOCAI_RESPONSE_ID_PK" PRIMARY KEY ("ID") USING INDEX ENABLE
);
CREATE
OR REPLACE EDITIONABLE TRIGGER "DOCAI_RESPONSE_BIU" BEFORE INSERT
OR
UPDATE
  ON DOCAI_RESPONSE FOR EACH ROW BEGIN IF INSERTING THEN : NEW.CREATED := SYSDATE;
: NEW.CREATED_BY := COALESCE(
  SYS_CONTEXT('APEX$SESSION', 'APP_USER'),
  USER
);
...
/ CREATE
OR REPLACE PROCEDURE UPLOAD_FILE (
  P_FILE_CONTENT IN VARCHAR2, P_STATIC_ID IN VARCHAR2,
  P_NAMESPACE IN VARCHAR2, P_BUCKET IN VARCHAR2,
  P_REGION IN VARCHAR2, P_OBJECT_STORAGE_URL OUT VARCHAR2,
  P_FILE_NAME OUT VARCHAR2, P_MIME_TYPE OUT VARCHAR2
) IS L_OBJECT BLOB;
L_RESPONSE CLOB;
BEGIN
SELECT
...
IF APEX_WEB_SERVICE.G_STATUS_CODE != 200 THEN RAISE_APPLICATION_ERROR(
  -20000, 'Upload failed - ' || APEX_WEB_SERVICE.G_STATUS_CODE
);
END IF;
END UPLOAD_FILE;
...
3. For Script Name: Enter **Application Scripts** and Click **Run**.
