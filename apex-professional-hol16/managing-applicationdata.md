...
3. Now, copy the statements below and execute them individually.
   ...
   (    "EMPNO" NUMBER GENERATED BY DEFAULT ON NULL
    ...
    ```
    <copy>
    insert into emp(empno, emp_name, mgr) values (10, 'JOHN', 30);
    </copy>
    ```

     ```
    <copy>
    insert into emp(empno, emp_name, mgr) values (20, 'CLARA',30);
    </copy>
    ```

     ```
    <copy>
    insert into emp(empno, emp_name, mgr) values (30, 'JANE', 40);
    </copy>
    ```

     ```
    <copy>
    insert into emp(empno, emp_name, mgr) values (40, 'MATT', null);
    </copy>
    ```
...
1. Log in to the first workspace, where we have already created multiple applications.
    ...
    ![Login to first Workspace](images/login-to-new-workspace.png " ")
