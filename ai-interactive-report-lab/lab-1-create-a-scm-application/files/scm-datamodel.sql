...
create table scm_warehouses (
    ...
    warehouse_type_code varchar2(20 char) not null
        annotations (
            display_label 'Warehouse Type',
            description 'Business classification of the warehouse.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include DISTRIBUTION, FULFILMENT, RETURNS, CROSS_DOCK, SPARES.'
        ),
    warehouse_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include ACTIVE, INACTIVE, CLOSED.'
        ),
    address_line_1 varchar2(200 char) not null
        annotations (
            display_label 'Address Line 1',
    ...
    constraint ck_scm_warehouses_01 check (warehouse_type_code in ('DISTRIBUTION', 'FULFILMENT', 'RETURNS', 'CROSS_DOCK', 'SPARES')),
    constraint ck_scm_warehouses_02 check (warehouse_status_code in ('ACTIVE', 'INACTIVE', 'CLOSED'))
) annotations (
...
create table scm_user_roles (
    ...
    constraint ck_scm_user_roles_01 check (role_scope_code in ('ENTERPRISE', 'WAREHOUSE', 'QUALITY', 'FINANCE', 'ADMIN')),
...
create table scm_application_users (
    ...
    can_receive_alerts boolean default true not null
        annotations (
            display_label 'Can Receive Alerts',
            description 'Indicates whether the user should receive replenishment, count, and exception alerts.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    approval_authority_level number(4) default 0 not null
        annotations (
            display_label 'Approval Authority Level',
            description 'Approval authority level granted directly to the user.',
            search_facet 'range',
            semantic_type 'ranking'
        ),
...
create table scm_user_role_assignments (
    ...
    assignment_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include ACTIVE, INACTIVE, SUSPENDED.'
        ),
    ...
    constraint uq_scm_user_role_asgn_01 unique (application_user_id, user_role_id, warehouse_id, effective_from_date),
    ...
    constraint fk_scm_user_role_asgn_04 foreign key (assigned_by_user_id)
        references scm_application_users (application_user_id),
    constraint ck_scm_user_role_asgn_01 check (assignment_status_code in ('ACTIVE', 'INACTIVE', 'SUSPENDED')),
    constraint ck_scm_user_role_asgn_02 check (authority_level_override is null or authority_level_override >= 0),
    constraint ck_scm_user_role_asgn_03 check (effective_to_date is null or effective_to_date >= effective_from_date)
) annotations (
...
create table scm_role_permissions (
    role_permission_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Role Permission ID',
            description 'Surrogate key for the role permission rule.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    user_role_id number not null
        annotations (
            display_label 'User Role ID',
            description 'Role that the permission applies to.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    process_area_code varchar2(30 char) not null
        annotations (
            display_label 'Process Area Code',
            description 'Warehouse process area covered by the permission.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'classification',
            ai_context 'Valid values include INBOUND, PUTAWAY, INVENTORY, REPLENISHMENT, OUTBOUND, RETURNS, QUARANTINE, COUNTING, APPROVALS, REPORTING, ADMINISTRATION.'
        ),
    access_level_code varchar2(20 char) not null
        annotations (
            display_label 'Access Level Code',
            description 'Business access level granted for the process area.',
            value_required 'true',
            semantic_type 'classification',
            ai_context 'Valid values include VIEW, WORK, SUPERVISE, APPROVE, ADMINISTER.'
        ),
    warehouse_scope_code varchar2(20 char) default 'ASSIGNED_WAREHOUSE' not null
        annotations (
            display_label 'Warehouse Scope Code',
            description 'Warehouse scope that the permission covers.',
            search_facet 'distinct-list',
            semantic_type 'scope',
            ai_context 'Valid values include ASSIGNED_WAREHOUSE, ALL_WAREHOUSES, COMPANY_WIDE.'
        ),
    is_active boolean default true not null
        annotations (
            display_label 'Is Active',
            description 'Indicates whether the permission rule is active.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    permission_notes varchar2(500 char)
        annotations (
            display_label 'Permission Notes',
            description 'Business notes for the permission rule.',
            semantic_type 'comment'
        ),
    constraint pk_scm_role_permissions primary key (role_permission_id),
    constraint uq_scm_role_permissions_01 unique (user_role_id, process_area_code, access_level_code, warehouse_scope_code),
    constraint fk_scm_role_permissions_01 foreign key (user_role_id)
        references scm_user_roles (user_role_id),
    constraint ck_scm_role_permissions_01 check (
        process_area_code in (
            'INBOUND',
            'PUTAWAY',
            'INVENTORY',
            'REPLENISHMENT',
    ...
    constraint ck_scm_role_permissions_02 check (access_level_code in ('VIEW', 'WORK', 'SUPERVISE', 'APPROVE', 'ADMINISTER')),
    constraint ck_scm_role_permissions_03 check (warehouse_scope_code in ('ASSIGNED_WAREHOUSE', 'ALL_WAREHOUSES', 'COMPANY_WIDE'))
) annotations (
    display_label 'Role Permissions',
...
create table scm_business_lists (
    ...
    list_code varchar2(30 char) not null
        annotations (
    ...
    list_purpose varchar2(500 char)
        annotations (
            display_label 'List Purpose',
            description 'Business purpose of the list and where it is used.',
            semantic_type 'description'
        ),
    maintained_by_role_id number
        annotations (
            display_label 'Maintained By Role ID',
            description 'Role allowed to maintain the list.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
...
create table scm_business_list_values (
    ...
    value_code varchar2(30 char) not null
        annotations (
    ...
    value_name varchar2(100 char) not null
        annotations (
    ...
    value_description varchar2(500 char)
        annotations (
            display_label 'Value Description',
            description 'Business meaning of the list value.',
            semantic_type 'description'
        ),
    ...
    is_default_value boolean default false not null
        annotations (
            display_label 'Is Default Value',
            description 'Indicates whether this is the default value for the list.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    is_active boolean default true not null
    ...
    constraint uq_scm_business_list_vals_01 unique (business_list_id, value_code),
    constraint fk_scm_business_list_vals_01 foreign key (business_list_id)
        references scm_business_lists (business_list_id),
    constraint ck_scm_business_list_vals_01 check (display_sequence is null or display_sequence > 0)
) annotations (
...
create table scm_approval_policies (
    approval_policy_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Approval Policy ID',
            description 'Surrogate key for the approval policy.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    policy_code varchar2(30 char) not null
        annotations (
            display_label 'Policy Code',
            description 'Business code used to identify the approval policy.',
            value_required 'true',
            semantic_type 'code'
        ),
    policy_name varchar2(150 char) not null
        annotations (
            display_label 'Policy Name',
            description 'Business name of the approval policy.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'name'
        ),
    approval_subject_code varchar2(30 char) not null
        annotations (
            display_label 'Approval Subject Code',
            description 'Business subject that the approval policy controls.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include STOCK_TRANSFER, STOCK_ADJUSTMENT, QUARANTINE_CASE, RECEIPT_REJECTION, COUNT_VARIANCE, ROTATION_EXCEPTION, OPERATIONAL_EXCEPTION.'
        ),
    warehouse_id number
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse where the policy applies where the rule is site specific.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    threshold_basis_code varchar2(20 char) not null
        annotations (
            display_label 'Threshold Basis Code',
            description 'Business basis used to decide when approval is required.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include ALWAYS, QUANTITY, VALUE, RISK, MIXED.'
        ),
    minimum_quantity number(18,4)
        annotations (
            display_label 'Minimum Quantity',
            description 'Minimum quantity threshold that triggers the policy where relevant.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    maximum_quantity number(18,4)
        annotations (
            display_label 'Maximum Quantity',
            description 'Maximum quantity threshold covered by the policy where relevant.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    minimum_value_amount number(18,2)
        annotations (
            display_label 'Minimum Value Amount',
            description 'Minimum stock value that triggers the policy where relevant.',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    maximum_value_amount number(18,2)
        annotations (
            display_label 'Maximum Value Amount',
            description 'Maximum stock value covered by the policy where relevant.',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    risk_level_code varchar2(20 char)
        annotations (
            display_label 'Risk Level Code',
            description 'Business risk level that triggers the policy where relevant.',
            semantic_type 'classification',
            ai_context 'Valid values include LOW, MEDIUM, HIGH, CRITICAL.'
        ),
    reviewer_role_id number not null
        annotations (
            display_label 'Reviewer Role ID',
            description 'Role that should review the approval request.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    escalation_role_id number
        annotations (
            display_label 'Escalation Role ID',
            description 'Role that should receive the request when it must be escalated.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    approval_authority_level number(4) default 1 not null
        annotations (
            display_label 'Approval Authority Level',
            description 'Authority level needed to act on the policy.',
            search_facet 'range',
            semantic_type 'ranking'
        ),
    policy_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            display_label 'Policy Status Code',
            description 'Current business status of the approval policy.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include ACTIVE, INACTIVE.'
        ),
    policy_notes varchar2(1000 char)
        annotations (
            display_label 'Policy Notes',
            description 'Business notes for the approval policy.',
            semantic_type 'comment'
        ),
    ...
    constraint fk_scm_approval_policies_03 foreign key (escalation_role_id)
        references scm_user_roles (user_role_id),
    constraint ck_scm_approval_policies_01 check (
        approval_subject_code in (
            'STOCK_TRANSFER',
            'STOCK_ADJUSTMENT',
            'QUARANTINE_CASE',
            'RECEIPT_REJECTION',
    ...
    constraint ck_scm_approval_policies_02 check (threshold_basis_code in ('ALWAYS', 'QUANTITY', 'VALUE', 'RISK', 'MIXED')),
    constraint ck_scm_approval_policies_03 check (minimum_quantity is null or minimum_quantity >= 0),
    constraint ck_scm_approval_policies_04 check (maximum_quantity is null or maximum_quantity >= 0),
    constraint ck_scm_approval_policies_05 check (minimum_value_amount is null or minimum_value_amount >= 0),
    constraint ck_scm_approval_policies_06 check (maximum_value_amount is null or maximum_value_amount >= 0),
    constraint ck_scm_approval_policies_07 check (risk_level_code in ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL') or risk_level_code is null),
    constraint ck_scm_approval_policies_08 check (approval_authority_level > 0),
    constraint ck_scm_approval_policies_09 check (policy_status_code in ('ACTIVE', 'INACTIVE')),
    constraint ck_scm_approval_policies_10 check (
        maximum_quantity is null
        or minimum_quantity is null
        or maximum_quantity >= minimum_quantity
    ),
    constraint ck_scm_approval_policies_11 check (
        maximum_value_amount is null
        or minimum_value_amount is null
        or maximum_value_amount >= minimum_value_amount
    )
) annotations (
    display_label 'Approval Policies',
    description 'Stores records for approval policies.',
    application_module 'Inventory and Warehouse Management'
);
...
create table scm_warehouse_areas (
    ...
    area_type_code varchar2(20 char) not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include RECEIVING, STORAGE, PICKING, PACKING, QUARANTINE, RETURNS, DISPATCH, STAGING.'
        ),
    ...
    allows_mixed_items boolean default false not null
        annotations (
            display_label 'Allows Mixed Items',
            description 'Indicates whether different items can be stored together in the same area.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    allows_damaged_stock boolean default false not null
        annotations (
            display_label 'Allows Damaged Stock',
            description 'Indicates whether damaged stock can be stored in this area.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    approved_for_high_value_flag boolean default false not null
        annotations (
            display_label 'Approved For High Value Flag',
            description 'Indicates whether the area is approved for high value stock.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    approved_for_sensitive_goods_flag boolean default false not null
        annotations (
            display_label 'Approved For Sensitive Goods Flag',
            description 'Indicates whether the area is approved for sensitive or controlled goods.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    approved_for_restricted_goods_flag boolean default false not null
        annotations (
            display_label 'Approved For Restricted Goods Flag',
            description 'Indicates whether the area is approved for restricted stock.',
            semantic_type 'flag'
        ),
    ...
    constraint fk_scm_warehouse_areas_01 foreign key (warehouse_id)
        references scm_warehouses (warehouse_id),
    constraint ck_scm_warehouse_areas_01 check (area_type_code in ('RECEIVING', 'STORAGE', 'PICKING', 'PACKING', 'QUARANTINE', 'RETURNS', 'DISPATCH', 'STAGING')),
    constraint ck_scm_warehouse_areas_02 check (area_status_code in ('ACTIVE', 'INACTIVE')),
    constraint ck_scm_warehouse_areas_03 check (sequence_number is null or sequence_number > 0)
) annotations (
...
create table scm_storage_locations (
    ...
    location_type_code varchar2(20 char) not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include DOCK, STAGING, BULK, PICK_FACE, BIN, PACK, DISPATCH, QUARANTINE, RETURNS.'
        ),
    ...
    max_capacity_units number(18,4)
        annotations (
            display_label 'Maximum Capacity Units',
            description 'Maximum quantity capacity supported by the location in the base handling unit.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    pick_sequence_number number(10)
        annotations (
            display_label 'Pick Sequence Number',
            description 'Sequence used to guide efficient picking within the warehouse.',
            semantic_type 'sequence'
        ),
    putaway_sequence_number number(10)
        annotations (
            display_label 'Putaway Sequence Number',
            description 'Sequence used to guide efficient putaway within the warehouse.',
            semantic_type 'sequence'
        ),
    approved_for_high_value_flag boolean default false not null
        annotations (
            display_label 'Approved For High Value Flag',
            description 'Indicates whether the location is approved for high value stock.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    approved_for_sensitive_goods_flag boolean default false not null
        annotations (
            display_label 'Approved For Sensitive Goods Flag',
            description 'Indicates whether the location is approved for sensitive or controlled goods.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    approved_for_restricted_goods_flag boolean default false not null
        annotations (
            display_label 'Approved For Restricted Goods Flag',
            description 'Indicates whether the location is approved for restricted stock.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    ...
    constraint ck_scm_storage_locations_01 check (location_type_code in ('DOCK', 'STAGING', 'BULK', 'PICK_FACE', 'BIN', 'PACK', 'DISPATCH', 'QUARANTINE', 'RETURNS')),
    constraint ck_scm_storage_locations_02 check (location_status_code in ('ACTIVE', 'INACTIVE', 'BLOCKED')),
    constraint ck_scm_storage_locations_03 check (max_capacity_units is null or max_capacity_units > 0),
    constraint ck_scm_storage_locations_04 check (pick_sequence_number is null or pick_sequence_number > 0),
    constraint ck_scm_storage_locations_05 check (putaway_sequence_number is null or putaway_sequence_number > 0)
) annotations (
...
create table scm_items (
    ...
    item_subcategory_code varchar2(30 char)
        annotations (
            display_label 'Item Subcategory Code',
            description 'Business subcategory of the item.',
            search_facet 'distinct-list',
            semantic_type 'category'
        ),
    item_family_code varchar2(30 char)
        annotations (
            display_label 'Item Family Code',
            description 'Business family used to group related items.',
            search_facet 'distinct-list',
            semantic_type 'classification'
        ),
    ...
    pack_uom_code varchar2(10 char)
        annotations (
            display_label 'Pack Unit of Measure',
            description 'Pack unit of measure used for case, carton, or handling quantities where relevant.',
            search_facet 'distinct-list',
            semantic_type 'unit_of_measure'
        ),
    item_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include ACTIVE, INACTIVE, DISCONTINUED.'
        ),
    handling_profile_code varchar2(30 char)
        annotations (
            display_label 'Handling Profile Code',
            description 'Business handling profile attached to the item.',
            search_facet 'distinct-list',
            semantic_type 'classification'
        ),
    storage_class_code varchar2(30 char)
        annotations (
            display_label 'Storage Class Code',
            description 'Business storage class used to guide approved storage areas and locations.',
            search_facet 'distinct-list',
            semantic_type 'classification'
        ),
    shelf_life_days number(6)
        annotations (
            display_label 'Shelf Life Days',
            description 'Expected shelf life of the item in days where relevant.',
            semantic_type 'duration_days'
        ),
    minimum_remaining_shelf_life_days number(6)
        annotations (
            display_label 'Minimum Remaining Shelf Life Days',
            description 'Minimum remaining shelf life required at the point of allocation or dispatch.',
            semantic_type 'duration_days'
        ),
    ...
    standard_weight_kg number(12,3)
        annotations (
            display_label 'Standard Weight KG',
            description 'Standard weight of one unit of the item in kilograms.',
            format_mask 'FM999G999G999G990D000',
            semantic_type 'quantity'
        ),
    standard_volume_cbm number(12,6)
        annotations (
            display_label 'Standard Volume CBM',
            description 'Standard volume of one unit of the item in cubic metres.',
            format_mask 'FM999G999G999G990D000000',
            semantic_type 'quantity'
        ),
    created_at timestamp with time zone default systimestamp not null
        annotations (
    ...
    constraint ck_scm_items_01 check (item_status_code in ('ACTIVE', 'INACTIVE', 'DISCONTINUED')),
    constraint ck_scm_items_02 check (shelf_life_days is null or shelf_life_days >= 0),
    constraint ck_scm_items_03 check (minimum_remaining_shelf_life_days is null or minimum_remaining_shelf_life_days >= 0),
    constraint ck_scm_items_04 check (
        shelf_life_days is null
        or minimum_remaining_shelf_life_days is null
        or minimum_remaining_shelf_life_days <= shelf_life_days
    ),
    constraint ck_scm_items_05 check (standard_weight_kg is null or standard_weight_kg >= 0),
    constraint ck_scm_items_06 check (standard_volume_cbm is null or standard_volume_cbm >= 0)
) annotations (
    display_label 'Items',
...
create table scm_item_warehouse_policies (
    ...
    rotation_method_code varchar2(10 char) not null
        annotations (
            display_label 'Rotation Method',
            description 'Stock rotation method used for the item in the warehouse.',
            value_required 'true',
            semantic_type 'method',
            ai_context 'Valid values include FIFO, FEFO.'
        ),
    ...
    maximum_stock_quantity number(18,4)
        annotations (
            display_label 'Maximum Stock Quantity',
            description 'Maximum planned stock quantity for the warehouse.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    ...
    count_tolerance_quantity number(18,4)
        annotations (
            display_label 'Count Tolerance Quantity',
            description 'Accepted quantity tolerance before a count difference needs investigation.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    count_tolerance_percent number(5,2)
        annotations (
            display_label 'Count Tolerance Percent',
            description 'Accepted percentage tolerance before a count difference needs investigation.',
            format_mask 'FM990D00',
            semantic_type 'percentage'
        ),
    cycle_count_class_code varchar2(1 char)
        annotations (
            display_label 'Cycle Count Class',
            description 'Counting priority class used to plan cycle counts.',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include A, B, C.'
        ),
    preferred_pick_area_id number
        annotations (
            display_label 'Preferred Pick Area ID',
            description 'Preferred warehouse area for picking this item.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    quarantine_on_receipt_flag boolean default false not null
        annotations (
            display_label 'Quarantine On Receipt Flag',
            description 'Indicates whether receipts should default to quarantine for this item in the warehouse.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    ...
    constraint uq_scm_item_wh_policies_01 unique (item_id, warehouse_id),
    ...
    constraint fk_scm_item_wh_policies_03 foreign key (preferred_pick_area_id)
        references scm_warehouse_areas (warehouse_area_id),
    constraint ck_scm_item_wh_policies_01 check (rotation_method_code in ('FIFO', 'FEFO')),
    constraint ck_scm_item_wh_policies_02 check (cycle_count_class_code in ('A', 'B', 'C') or cycle_count_class_code is null),
    constraint ck_scm_item_wh_policies_03 check (minimum_stock_quantity is null or minimum_stock_quantity >= 0),
    constraint ck_scm_item_wh_policies_04 check (reorder_point_quantity is null or reorder_point_quantity >= 0),
    constraint ck_scm_item_wh_policies_05 check (reorder_target_quantity is null or reorder_target_quantity >= 0),
    constraint ck_scm_item_wh_policies_06 check (maximum_stock_quantity is null or maximum_stock_quantity >= 0),
    constraint ck_scm_item_wh_policies_06a check (safety_stock_quantity is null or safety_stock_quantity >= 0),
    constraint ck_scm_item_wh_policies_06b check (replenishment_lead_time_days is null or replenishment_lead_time_days >= 0),
    constraint ck_scm_item_wh_policies_07 check (
        reorder_point_quantity is null
        or reorder_target_quantity is null
        or reorder_point_quantity <= reorder_target_quantity
    ),
    constraint ck_scm_item_wh_policies_08 check (
        reorder_target_quantity is null
        or maximum_stock_quantity is null
        or reorder_target_quantity <= maximum_stock_quantity
    ),
    constraint ck_scm_item_wh_policies_08a check (
        safety_stock_quantity is null
        or reorder_point_quantity is null
        or safety_stock_quantity <= reorder_point_quantity
    ),
    constraint ck_scm_item_wh_policies_09 check (count_tolerance_quantity is null or count_tolerance_quantity >= 0),
    constraint ck_scm_item_wh_policies_10 check (
        count_tolerance_percent is null
        or (count_tolerance_percent >= 0 and count_tolerance_percent <= 100)
    )
) annotations (
...
create table scm_replenishment_alerts (
    ...
    constraint ck_scm_replenishment_alerts_02 check (alert_status_code in ('OPEN', 'IN_REVIEW', 'ACTIONED', 'CLOSED', 'SUPPRESSED')),
    constraint ck_scm_replenishment_alerts_03 check (priority_code in ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL')),
...
create table scm_inventory_lots (
    ...
    lot_number varchar2(60 char) not null
        annotations (
    ...
    manufactured_on_date date
        annotations (
            display_label 'Manufactured On Date',
            description 'Manufacturing date of the lot where known.',
            format_mask 'DD-MON-YYYY',
            semantic_type 'date'
        ),
    ...
    lot_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include ACTIVE, QUARANTINED, BLOCKED, EXPIRED, CLOSED.'
        ),
    ...
    constraint fk_scm_inventory_lots_01 foreign key (item_id)
        references scm_items (item_id),
    constraint ck_scm_inventory_lots_01 check (lot_status_code in ('ACTIVE', 'QUARANTINED', 'BLOCKED', 'EXPIRED', 'CLOSED')),
    constraint ck_scm_inventory_lots_02 check (
...
create table scm_inventory_balances (
    ...
    stock_status_code varchar2(20 char) not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include AVAILABLE, QUARANTINED, DAMAGED, BLOCKED, RETURN_HOLD.'
        ),
    quantity_on_hand number(18,4) default 0 not null
        annotations (
    ...
    quantity_reserved number(18,4) default 0 not null
        annotations (
    ...
    quantity_available number(18,4) generated always as (
        case
            when stock_status_code = 'AVAILABLE' then quantity_on_hand - quantity_reserved
            else 0
        end
    ) virtual
        annotations (
    ...
    lot_key number generated always as (coalesce(inventory_lot_id, 0)) virtual
        annotations (
            display_label 'Lot Key',
            description 'Derived key used to enforce uniqueness where a lot is optional.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    constraint pk_scm_inventory_balances primary key (inventory_balance_id),
    constraint uq_scm_inventory_balances_01 unique (
        storage_location_id,
        item_id,
        lot_key,
        stock_status_code
    ...
    constraint ck_scm_inventory_balances_01 check (stock_status_code in ('AVAILABLE', 'QUARANTINED', 'DAMAGED', 'BLOCKED', 'RETURN_HOLD')),
...
create table scm_inventory_transactions (
    inventory_transaction_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Inventory Transaction ID',
            description 'Surrogate key for the inventory transaction.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    warehouse_id number not null
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse where the transaction was recorded.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    transaction_type_code varchar2(30 char) not null
        annotations (
            display_label 'Transaction Type',
            description 'Business event that changed stock or stock status.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include RECEIPT, PUTAWAY, MOVE, TRANSFER_OUT, TRANSFER_IN, RESERVE, RESERVE_RELEASE, PICK, PACK, DISPATCH, RETURN, ADJUSTMENT, COUNT_VARIANCE, QUARANTINE, QUARANTINE_RELEASE.'
        ),
    from_location_id number
        annotations (
            display_label 'From Location ID',
            description 'Location that stock moved from where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    to_location_id number
        annotations (
            display_label 'To Location ID',
            description 'Location that stock moved to where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    item_id number not null
        annotations (
            display_label 'Item ID',
            description 'Item affected by the transaction.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    inventory_lot_id number
        annotations (
            display_label 'Inventory Lot ID',
            description 'Lot affected by the transaction where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    from_status_code varchar2(20 char)
        annotations (
            ...
            description 'Stock status before the transaction where relevant.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include AVAILABLE, QUARANTINED, DAMAGED, BLOCKED, RETURN_HOLD.'
        ),
    to_status_code varchar2(20 char)
        annotations (
            ...
            description 'Stock status after the transaction where relevant.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include AVAILABLE, QUARANTINED, DAMAGED, BLOCKED, RETURN_HOLD.'
        ),
    quantity number(18,4) not null
        annotations (
            ...
            description 'Quantity moved or changed by the transaction.',
            format_mask 'FM999G999G999G990D0000',
            value_required 'true',
            semantic_type 'quantity'
        ),
    reference_document_type varchar2(30 char)
        annotations (
            display_label 'Reference Document Type',
            description 'Type of business document that caused the transaction.',
            semantic_type 'type'
        ),
    reference_document_number varchar2(50 char)
        annotations (
            display_label 'Reference Document Number',
            description 'Business reference number of the document that caused the transaction.',
            primary_display_column 'true',
            semantic_type 'reference_number'
        ),
    reason_code varchar2(30 char)
        annotations (
            ...
            description 'Business reason code for the transaction.',
            semantic_type 'reason_code'
        ),
    reason_description varchar2(500 char)
        annotations (
            display_label 'Reason Description',
            description 'Additional business explanation for the transaction.',
            semantic_type 'description'
        ),
    transaction_at timestamp with time zone default systimestamp not null
        annotations (
            display_label 'Transaction At',
            description 'Date and time when the inventory transaction was recorded.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    performed_by varchar2(128 char) not null
        annotations (
            display_label 'Performed By',
            description 'User or process that recorded the transaction.',
            value_required 'true',
            semantic_type 'user_name'
        ),
    ...
    constraint fk_scm_inventory_txns_05 foreign key (inventory_lot_id)
        references scm_inventory_lots (inventory_lot_id),
    constraint ck_scm_inventory_txns_01 check (
    ...
    constraint ck_scm_inventory_txns_03 check (
        from_status_code in ('AVAILABLE', 'QUARANTINED', 'DAMAGED', 'BLOCKED', 'RETURN_HOLD')
        or from_status_code is null
    ),
    constraint ck_scm_inventory_txns_04 check (
        to_status_code in ('AVAILABLE', 'QUARANTINED', 'DAMAGED', 'BLOCKED', 'RETURN_HOLD')
        or to_status_code is null
    ),
    constraint ck_scm_inventory_txns_05 check (
        transaction_type_code not in ('RESERVE', 'PICK', 'PACK', 'DISPATCH')
        or from_status_code = 'AVAILABLE'
    )
) annotations (
    display_label 'Inventory Transactions',
    description 'Stores records for inventory transactions.',
    application_module 'Inventory and Warehouse Management'
);
...
create table scm_inbound_receipts (
    ...
    receipt_status_code varchar2(20 char) default 'PLANNED' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include PLANNED, ARRIVED, PART_RECEIVED, RECEIVED, CANCELLED, CLOSED.'
        ),
    ...
    carrier_reference varchar2(50 char)
        annotations (
            display_label 'Carrier Reference',
            description 'Carrier or transport reference for the inbound delivery.',
            semantic_type 'reference_number'
        ),
    ...
    constraint ck_scm_inbound_receipts_02 check (receipt_status_code in ('PLANNED', 'ARRIVED', 'PART_RECEIVED', 'RECEIVED', 'CANCELLED', 'CLOSED'))
) annotations (
...
create table scm_inbound_receipt_lines (
    ...
    suggested_putaway_location_id number
        annotations (
            display_label 'Suggested Putaway Location ID',
            description 'Suggested storage location for putaway.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    ...
    expected_quantity number(18,4) default 0 not null
        annotations (
    ...
    received_quantity number(18,4) default 0 not null
        annotations (
    ...
    received_condition_code varchar2(20 char)
        annotations (
            ...
            ai_context 'Valid values include GOOD, DAMAGED, MIXED, SUSPECT.'
        ),
    accepted_quantity number(18,4) default 0 not null
        annotations (
    ...
    quarantine_quantity number(18,4) default 0 not null
        annotations (
    ...
    damaged_quantity number(18,4) default 0 not null
        annotations (
    ...
    shortage_quantity number(18,4) default 0 not null
        annotations (
    ...
    overage_quantity number(18,4) default 0 not null
        annotations (
    ...
    rejected_quantity number(18,4) default 0 not null
        annotations (
    ...
    rejected_disposition_code varchar2(30 char)
        annotations (
            display_label 'Rejected Disposition Code',
            description 'Business outcome applied to the rejected quantity.',
            semantic_type 'disposition',
            ai_context 'Valid values include RETURN_TO_SOURCE, REFUSE_ON_DOCK, HOLD_FOR_CREDIT, DISPOSE.'
        ),
    line_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, RECEIVED, PUTAWAY_PENDING, PUTAWAY_COMPLETE, CANCELLED.'
        ),
    ...
    line_notes varchar2(500 char)
        annotations (
            display_label 'Line Notes',
            description 'Business notes recorded against the receipt line.',
            semantic_type 'comment'
        ),
    ...
    constraint ck_scm_inbound_receipt_lines_09 check (
        accepted_quantity + quarantine_quantity + damaged_quantity + rejected_quantity <= received_quantity
    ),
    constraint ck_scm_inbound_receipt_lines_10 check (line_status_code in ('OPEN', 'RECEIVED', 'PUTAWAY_PENDING', 'PUTAWAY_COMPLETE', 'CANCELLED')),
    constraint ck_scm_inbound_receipt_lines_11 check (received_condition_code in ('GOOD', 'DAMAGED', 'MIXED', 'SUSPECT') or received_condition_code is null),
    constraint ck_scm_inbound_receipt_lines_12 check (rejected_quantity >= 0),
    constraint ck_scm_inbound_receipt_lines_13 check (
        rejected_disposition_code in ('RETURN_TO_SOURCE', 'REFUSE_ON_DOCK', 'HOLD_FOR_CREDIT', 'DISPOSE')
...
create table scm_stock_transfers (
    ...
    transfer_status_code varchar2(20 char) default 'DRAFT' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include DRAFT, PENDING_APPROVAL, RELEASED, IN_TRANSIT, PART_RECEIVED, COMPLETED, CANCELLED.'
        ),
    ...
    confirmation_due_at timestamp with time zone
        annotations (
            display_label 'Confirmation Due At',
            description 'Date and time when transfer receipt confirmation is due before overdue escalation should be raised.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    ...
    requested_by varchar2(128 char) not null
        annotations (
            ...
            description 'User who requested the stock transfer.',
            value_required 'true',
            semantic_type 'user_name'
        ),
    approved_by varchar2(128 char)
        annotations (
            display_label 'Approved By',
            description 'User who approved the stock transfer where approval was needed.',
            semantic_type 'user_name'
        ),
    approved_at timestamp with time zone
        annotations (
            display_label 'Approved At',
            description 'Date and time when the transfer was approved.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    approval_policy_id number
        annotations (
            display_label 'Approval Policy ID',
            description 'Approval policy applied to the transfer where approval was required.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    ...
    constraint ck_scm_stock_transfers_02 check (transfer_status_code in ('DRAFT', 'PENDING_APPROVAL', 'RELEASED', 'IN_TRANSIT', 'PART_RECEIVED', 'COMPLETED', 'CANCELLED')),
    constraint ck_scm_stock_transfers_03 check (
        expected_receipt_at is null
        or actual_shipped_at is null
        or expected_receipt_at >= actual_shipped_at
    ),
    constraint ck_scm_stock_transfers_04 check (
...
create table scm_stock_transfer_lines (
    ...
    requested_quantity number(18,4) not null
        annotations (
    ...
    shipped_quantity number(18,4) default 0 not null
        annotations (
    ...
    quantity_discrepancy number(18,4) generated always as (shipped_quantity - received_quantity) virtual
        annotations (
    ...
    unit_cost_amount number(18,2)
        annotations (
            display_label 'Unit Cost Amount',
            description 'Unit cost captured for the transfer line where financial exposure is relevant.',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    currency_code varchar2(3 char) default 'INR' not null
        annotations (
            display_label 'Currency Code',
            description 'Currency used for the transfer line cost values.',
            semantic_type 'currency_code'
        ),
    shipped_line_value_amount number(18,2) generated always as (
        round(shipped_quantity * nvl(unit_cost_amount, 0), 2)
    ) virtual
        annotations (
            display_label 'Shipped Line Value Amount',
            description 'Estimated financial value of the shipped quantity.',
            display_in_form 'false',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    received_line_value_amount number(18,2) generated always as (
        round(received_quantity * nvl(unit_cost_amount, 0), 2)
    ) virtual
        annotations (
            display_label 'Received Line Value Amount',
            description 'Estimated financial value of the received quantity.',
            display_in_form 'false',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    transfer_discrepancy_reason_code varchar2(30 char)
        annotations (
            display_label 'Transfer Discrepancy Reason Code',
            description 'Business reason code used when received quantity differs from shipped quantity.',
            semantic_type 'reason_code'
        ),
    transfer_discrepancy_notes varchar2(500 char)
        annotations (
            display_label 'Transfer Discrepancy Notes',
            description 'Business notes recorded for a transfer discrepancy.',
            semantic_type 'comment'
        ),
    source_stock_status_code varchar2(20 char) default 'AVAILABLE' not null
        annotations (
            display_label 'Source Stock Status Code',
            description 'Stock status allowed to supply the transfer line in the current release.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Only AVAILABLE is valid for warehouse-to-warehouse transfer supply.'
        ),
    source_lot_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            display_label 'Source Lot Status Code',
            description 'Lot status allowed to supply the transfer line where lot control applies in the current release.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Only ACTIVE is valid for warehouse-to-warehouse transfer supply.'
        ),
    line_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            display_label 'Line Status',
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, SHIPPED, PART_RECEIVED, RECEIVED, CANCELLED.'
        ),
    ...
    constraint ck_scm_stock_transfer_lines_07 check (line_status_code in ('OPEN', 'SHIPPED', 'PART_RECEIVED', 'RECEIVED', 'CANCELLED')),
    ...
    constraint ck_scm_stock_transfer_lines_10 check (unit_cost_amount is null or unit_cost_amount >= 0)
) annotations (
...
create table scm_outbound_orders (
    ...
    outbound_status_code varchar2(20 char) default 'NEW' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include NEW, RELEASED, ALLOCATED, PICKING, PICKED, PACKED, PART_DISPATCHED, DISPATCHED, CANCELLED, CLOSED.'
        ),
    source_document_number varchar2(50 char)
        annotations (
            display_label 'Source Document Number',
    ...
    promised_ship_at timestamp with time zone
        annotations (
            display_label 'Promised Ship At',
            description 'Promised shipment date and time.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    partial_fulfilment_allowed_flag boolean default false not null
        annotations (
            display_label 'Partial Fulfilment Allowed Flag',
            description 'Indicates whether the order may be dispatched in part where stock is short.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    picked_at timestamp with time zone
        annotations (
            ...
            description 'Date and time when picking was completed.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    picked_by varchar2(128 char)
        annotations (
            display_label 'Picked By',
            description 'User who completed picking for the outbound order.',
            semantic_type 'user_name'
        ),
    packed_at timestamp with time zone
        annotations (
            display_label 'Packed At',
            description 'Date and time when packing was completed.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    packed_by varchar2(128 char)
        annotations (
            display_label 'Packed By',
            description 'User who completed packing for the outbound order.',
            semantic_type 'user_name'
        ),
    ...
    dispatched_by varchar2(128 char)
        annotations (
            display_label 'Dispatched By',
            description 'User who confirmed dispatch of the outbound order.',
            semantic_type 'user_name'
        ),
    package_count number(10)
        annotations (
            display_label 'Package Count',
            description 'Number of packages prepared for dispatch.',
            semantic_type 'count'
        ),
    notes varchar2(1000 char)
        annotations (
    ...
    constraint ck_scm_outbound_orders_02 check (outbound_status_code in ('NEW', 'RELEASED', 'ALLOCATED', 'PICKING', 'PICKED', 'PACKED', 'PART_DISPATCHED', 'DISPATCHED', 'CANCELLED', 'CLOSED')),
    constraint ck_scm_outbound_orders_03 check (package_count is null or package_count >= 0)
) annotations (
...
create table scm_outbound_order_lines (
    ...
    reserved_quantity number(18,4) default 0 not null
        annotations (
    ...
    picked_quantity number(18,4) default 0 not null
        annotations (
    ...
    packed_quantity number(18,4) default 0 not null
        annotations (
    ...
    dispatched_quantity number(18,4) default 0 not null
        annotations (
    ...
    short_quantity number(18,4) default 0 not null
        annotations (
    ...
    source_stock_status_code varchar2(20 char) default 'AVAILABLE' not null
        annotations (
            display_label 'Source Stock Status Code',
            description 'Stock status allowed to supply reservation, picking, packing, and dispatch in the current release.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Only AVAILABLE is valid for normal outbound fulfilment.'
        ),
    source_lot_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            display_label 'Source Lot Status Code',
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Only ACTIVE is valid for normal outbound fulfilment.'
        ),
    rotation_exception_flag boolean default false not null
        annotations (
            display_label 'Rotation Exception Flag',
            description 'Indicates whether the line used a stock rotation exception.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    rotation_exception_reason_code varchar2(30 char)
        annotations (
            display_label 'Rotation Exception Reason Code',
            description 'Business reason for using a stock rotation exception.',
            semantic_type 'reason_code'
        ),
    rotation_approval_policy_id number
        annotations (
            display_label 'Rotation Approval Policy ID',
            description 'Approval policy applied to the stock rotation exception where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    rotation_approved_by varchar2(128 char)
        annotations (
            display_label 'Rotation Approved By',
            description 'User who approved the stock rotation exception.',
            semantic_type 'user_name'
        ),
    rotation_approved_at timestamp with time zone
        annotations (
            display_label 'Rotation Approved At',
            description 'Date and time when the stock rotation exception was approved.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    allocation_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, ALLOCATED, PART_ALLOCATED, PICKED, PACKED, PART_DISPATCHED, DISPATCHED, SHORT, CANCELLED.'
        ),
    ...
    constraint ck_scm_outbound_order_lines_12 check (allocation_status_code in ('OPEN', 'ALLOCATED', 'PART_ALLOCATED', 'PICKED', 'PACKED', 'PART_DISPATCHED', 'DISPATCHED', 'SHORT', 'CANCELLED')),
    ...
    constraint ck_scm_outbound_order_lines_15 check (
        rotation_exception_flag = false
        or (
            rotation_exception_reason_code is not null
...
create table scm_item_serials (
    ...
    source_inbound_receipt_line_id number
        annotations (
            display_label 'Source Inbound Receipt Line ID',
            description 'Inbound receipt line where the serial number first entered warehouse control.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    current_status_code varchar2(20 char) default 'AVAILABLE' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include AVAILABLE, RESERVED, PICKED, DISPATCHED, QUARANTINED, DAMAGED, BLOCKED, RETURN_HOLD, WRITTEN_OFF.'
        ),
    serial_condition_code varchar2(20 char) default 'GOOD' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'condition',
            ai_context 'Valid values include GOOD, DAMAGED, SUSPECT, EXPIRED, RETURNED.'
        ),
    ...
    serial_notes varchar2(500 char)
        annotations (
            display_label 'Serial Notes',
            description 'Business notes for the serial record.',
            semantic_type 'comment'
        ),
    ...
    constraint ck_scm_item_serials_01 check (
    ...
    constraint ck_scm_item_serials_02 check (serial_condition_code in ('GOOD', 'DAMAGED', 'SUSPECT', 'EXPIRED', 'RETURNED'))
) annotations (
...
create table scm_serial_events (
    serial_event_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Serial Event ID',
            description 'Surrogate key for the serial event.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    item_serial_id number not null
        annotations (
            display_label 'Item Serial ID',
            description 'Serial number affected by the event.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    warehouse_id number
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse recording the serial event where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    from_location_id number
        annotations (
            display_label 'From Location ID',
            description 'Location the serial number moved from where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    to_location_id number
        annotations (
            display_label 'To Location ID',
            description 'Location the serial number moved to where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    event_type_code varchar2(30 char) not null
        annotations (
            display_label 'Event Type Code',
            description 'Business event recorded for the serial number.',
            value_required 'true',
            primary_display_column 'true',
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include RECEIPT, PUTAWAY, MOVE, RESERVE, PICK, PACK, DISPATCH, RETURN, QUARANTINE, QUARANTINE_RELEASE, ADJUSTMENT, COUNT.'
        ),
    from_status_code varchar2(20 char)
        annotations (
            display_label 'From Status Code',
            description 'Status before the serial event where relevant.',
            search_facet 'distinct-list',
            semantic_type 'status'
        ),
    to_status_code varchar2(20 char)
        annotations (
            display_label 'To Status Code',
            description 'Status after the serial event where relevant.',
            search_facet 'distinct-list',
            semantic_type 'status'
        ),
    reference_document_type varchar2(30 char)
        annotations (
            display_label 'Reference Document Type',
            description 'Business document type that caused the serial event.',
            semantic_type 'type'
        ),
    reference_document_number varchar2(50 char)
        annotations (
            display_label 'Reference Document Number',
            description 'Business document number that caused the serial event.',
            semantic_type 'reference_number'
        ),
    event_at timestamp with time zone default systimestamp not null
        annotations (
            display_label 'Event At',
            description 'Date and time when the serial event was recorded.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    performed_by varchar2(128 char) not null
        annotations (
            display_label 'Performed By',
            description 'User or process that recorded the serial event.',
            value_required 'true',
            semantic_type 'user_name'
        ),
    event_notes varchar2(1000 char)
        annotations (
            display_label 'Event Notes',
            description 'Business notes recorded for the serial event.',
            semantic_type 'comment'
        ),
    ...
    constraint fk_scm_serial_events_04 foreign key (to_location_id)
        references scm_storage_locations (storage_location_id),
    constraint ck_scm_serial_events_01 check (
        event_type_code in (
            'RECEIPT',
            'PUTAWAY',
            'MOVE',
    ...
    constraint ck_scm_serial_events_02 check (
        from_status_code in (
            'AVAILABLE',
            'RESERVED',
            'PICKED',
    ...
    constraint ck_scm_serial_events_03 check (
        to_status_code in (
            'AVAILABLE',
            'RESERVED',
            'PICKED',
    ...
    display_label 'Serial Events',
    description 'Stores records for serial events.',
    application_module 'Inventory and Warehouse Management'
);
...
create table scm_returns (
    ...
    return_type_code varchar2(20 char) not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include CUSTOMER_RETURN, DELIVERY_RETURN, INTERNAL_RETURN.'
        ),
    warehouse_id number not null
        annotations (
            display_label 'Warehouse ID',
    ...
    return_status_code varchar2(20 char) default 'EXPECTED' not null
        annotations (
            display_label 'Return Status',
            description 'Current business status of the return.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include EXPECTED, RECEIVED, ASSESSED, CLOSED, CANCELLED.'
        ),
    ...
    received_by varchar2(128 char)
        annotations (
            display_label 'Received By',
            description 'User who checked the returned goods on arrival.',
            semantic_type 'user_name'
        ),
    ...
    assessed_by varchar2(128 char)
        annotations (
            display_label 'Assessed By',
            description 'User who assessed the return outcome.',
            semantic_type 'user_name'
        ),
    notes varchar2(1000 char)
        annotations (
    ...
    constraint fk_scm_returns_04 foreign key (related_outbound_order_id)
        references scm_outbound_orders (outbound_order_id),
    constraint ck_scm_returns_01 check (return_type_code in ('CUSTOMER_RETURN', 'DELIVERY_RETURN', 'INTERNAL_RETURN')),
    constraint ck_scm_returns_02 check (return_status_code in ('EXPECTED', 'RECEIVED', 'ASSESSED', 'CLOSED', 'CANCELLED'))
) annotations (
...
create table scm_return_lines (
    ...
    reusable_quantity number(18,4) default 0 not null
        annotations (
    ...
    writeoff_quantity number(18,4) default 0 not null
        annotations (
    ...
    disposition_code varchar2(20 char)
        annotations (
            ...
            semantic_type 'disposition',
            ai_context 'Valid values include RETURN_TO_STOCK, QUARANTINE, RETURN_TO_SUPPLIER, WRITE_OFF, REWORK.'
        ),
    ...
    constraint ck_scm_return_lines_08 check (reusable_quantity + quarantine_quantity + damaged_quantity + writeoff_quantity <= received_quantity),
    constraint ck_scm_return_lines_09 check (disposition_code in ('RETURN_TO_STOCK', 'QUARANTINE', 'RETURN_TO_SUPPLIER', 'WRITE_OFF', 'REWORK') or disposition_code is null),
    constraint ck_scm_return_lines_10 check (returned_condition_code in ('GOOD', 'DAMAGED', 'INCOMPLETE', 'EXPIRED', 'SUSPECT') or returned_condition_code is null)
) annotations (
...
create table scm_quarantine_cases (
    quarantine_case_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Quarantine Case ID',
            description 'Surrogate key for the quarantine case.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    quarantine_number varchar2(30 char) not null
        annotations (
            display_label 'Quarantine Number',
            description 'Business reference used to identify the quarantine case.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'reference_number'
        ),
    warehouse_id number not null
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse where the quarantined stock is held.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    storage_location_id number not null
        annotations (
            display_label 'Storage Location ID',
            description 'Location where the quarantined stock is held.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    source_document_type varchar2(30 char) not null
        annotations (
            display_label 'Source Document Type',
            description 'Type of business document that created the quarantine case.',
            value_required 'true',
            semantic_type 'type'
        ),
    source_document_number varchar2(50 char)
        annotations (
            display_label 'Source Document Number',
            description 'Business number of the document that created the quarantine case.',
            semantic_type 'reference_number'
        ),
    item_id number not null
        annotations (
            display_label 'Item ID',
            description 'Item placed into quarantine.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    inventory_lot_id number
        annotations (
            display_label 'Inventory Lot ID',
            description 'Lot placed into quarantine where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    quantity number(18,4) not null
        annotations (
            display_label 'Quantity',
            description 'Quantity currently controlled by the quarantine case.',
            format_mask 'FM999G999G999G990D0000',
            value_required 'true',
            semantic_type 'quantity'
        ),
    quarantine_reason_code varchar2(30 char) not null
        annotations (
            display_label 'Quarantine Reason Code',
            description 'Business reason that stock was quarantined.',
            value_required 'true',
            semantic_type 'reason_code'
        ),
    quarantine_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            display_label 'Quarantine Status',
            description 'Current business status of the quarantine case.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, UNDER_REVIEW, RELEASED, RETURNED, REWORKED, WRITTEN_OFF, CLOSED.'
        ),
    quarantined_at timestamp with time zone default systimestamp not null
        annotations (
            display_label 'Quarantined At',
            description 'Date and time when the stock entered quarantine.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    reviewed_at timestamp with time zone
        annotations (
            display_label 'Reviewed At',
            description 'Date and time when the quarantine case was reviewed.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    reviewed_by varchar2(128 char)
        annotations (
            ...
            description 'User who reviewed the quarantine case.',
            semantic_type 'user_name'
        ),
    resolution_code varchar2(30 char)
        annotations (
            display_label 'Resolution Code',
            description 'Business resolution applied to the quarantine case.',
            semantic_type 'resolution'
        ),
    resolution_notes varchar2(1000 char)
        annotations (
            display_label 'Resolution Notes',
            description 'Business notes explaining the quarantine outcome.',
            semantic_type 'comment'
        ),
    ...
    constraint ck_scm_quarantine_cases_02 check (quarantine_status_code in ('OPEN', 'UNDER_REVIEW', 'RELEASED', 'RETURNED', 'REWORKED', 'WRITTEN_OFF', 'CLOSED'))
) annotations (
    display_label 'Quarantine Cases',
    description 'Stores records for quarantine cases.',
    application_module 'Inventory and Warehouse Management'
);
...
create table scm_stock_adjustments (
    ...
    adjustment_type_code varchar2(20 char) not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include GAIN, LOSS, STATUS_CHANGE, WRITE_OFF.'
        ),
    adjustment_status_code varchar2(20 char) default 'DRAFT' not null
        annotations (
            display_label 'Adjustment Status',
            description 'Current business status of the stock adjustment.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include DRAFT, PENDING_APPROVAL, APPROVED, APPLIED, REJECTED, CANCELLED.'
        ),
    reason_code varchar2(30 char) not null
        annotations (
            display_label 'Reason Code',
    ...
    requested_by varchar2(128 char) not null
        annotations (
            display_label 'Requested By',
            description 'User who requested the stock adjustment.',
            value_required 'true',
            semantic_type 'user_name'
        ),
    ...
    approved_by varchar2(128 char)
        annotations (
            display_label 'Approved By',
            description 'User who approved the stock adjustment where approval was needed.',
            semantic_type 'user_name'
        ),
    approved_at timestamp with time zone
        annotations (
            display_label 'Approved At',
            description 'Date and time when the stock adjustment was approved.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    approval_policy_id number
        annotations (
            display_label 'Approval Policy ID',
            description 'Approval policy applied to the stock adjustment where approval was required.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    ...
    constraint fk_scm_stock_adjustments_02 foreign key (approval_policy_id)
        references scm_approval_policies (approval_policy_id),
    constraint ck_scm_stock_adjustments_01 check (adjustment_type_code in ('GAIN', 'LOSS', 'STATUS_CHANGE', 'WRITE_OFF')),
    constraint ck_scm_stock_adjustments_02 check (adjustment_status_code in ('DRAFT', 'PENDING_APPROVAL', 'APPROVED', 'APPLIED', 'REJECTED', 'CANCELLED'))
) annotations (
...
create table scm_stock_adjustment_lines (
    ...
    adjustment_direction_code varchar2(20 char) not null
        annotations (
            ...
            ai_context 'Valid values include INCREASE, DECREASE, RECLASSIFY.'
        ),
    adjustment_quantity number(18,4) not null
        annotations (
    ...
    unit_cost_amount number(18,2)
        annotations (
            display_label 'Unit Cost Amount',
            description 'Unit cost captured for the adjustment line where financial review is relevant.',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    currency_code varchar2(3 char) default 'INR' not null
        annotations (
            display_label 'Currency Code',
            description 'Currency used for the adjustment line value.',
            semantic_type 'currency_code'
        ),
    line_value_amount number(18,2) generated always as (
        round(adjustment_quantity * nvl(unit_cost_amount, 0), 2)
    ) virtual
        annotations (
            display_label 'Line Value Amount',
            description 'Estimated financial value of the adjustment line.',
            display_in_form 'false',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    ...
    constraint uq_scm_stock_adj_lines_01 unique (stock_adjustment_id, line_number),
    ...
    constraint ck_scm_stock_adj_lines_02 check (adjustment_direction_code in ('INCREASE', 'DECREASE', 'RECLASSIFY')),
    constraint ck_scm_stock_adj_lines_03 check (adjustment_quantity > 0),
    constraint ck_scm_stock_adj_lines_04 check (
        from_status_code in ('AVAILABLE', 'QUARANTINED', 'DAMAGED', 'BLOCKED', 'RETURN_HOLD')
        or from_status_code is null
    ),
    constraint ck_scm_stock_adj_lines_05 check (
        to_status_code in ('AVAILABLE', 'QUARANTINED', 'DAMAGED', 'BLOCKED', 'RETURN_HOLD')
        or to_status_code is null
    ),
    constraint ck_scm_stock_adj_lines_06 check (unit_cost_amount is null or unit_cost_amount >= 0)
) annotations (
...
create table scm_stock_counts (
    ...
    count_type_code varchar2(20 char) not null
        annotations (
            display_label 'Count Type',
            description 'Business type of the stock count.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include CYCLE, FULL, SPOT.'
        ),
    ...
    count_status_code varchar2(20 char) default 'PLANNED' not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include PLANNED, IN_PROGRESS, REVIEW, CLOSED, CANCELLED.'
        ),
    ...
    requested_by varchar2(128 char) not null
        annotations (
            display_label 'Requested By',
            description 'User who requested or scheduled the count.',
            value_required 'true',
            semantic_type 'user_name'
        ),
    reviewed_by varchar2(128 char)
        annotations (
            display_label 'Reviewed By',
            description 'User who reviewed the count outcome.',
            semantic_type 'user_name'
        ),
    notes varchar2(1000 char)
        annotations (
    ...
    constraint fk_scm_stock_counts_01 foreign key (warehouse_id)
        references scm_warehouses (warehouse_id),
    constraint ck_scm_stock_counts_01 check (count_type_code in ('CYCLE', 'FULL', 'SPOT')),
    constraint ck_scm_stock_counts_02 check (count_scope_code in ('WAREHOUSE', 'AREA', 'LOCATION', 'ITEM')),
    constraint ck_scm_stock_counts_03 check (count_status_code in ('PLANNED', 'IN_PROGRESS', 'REVIEW', 'CLOSED', 'CANCELLED'))
) annotations (
...
create table scm_stock_count_lines (
    ...
    counted_quantity number(18,4) default 0 not null
        annotations (
    ...
    variance_quantity number(18,4) generated always as (counted_quantity - expected_quantity) virtual
        annotations (
    ...
    unit_cost_amount number(18,2)
        annotations (
            display_label 'Unit Cost Amount',
            description 'Unit cost captured for the counted item where variance value matters for review.',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    currency_code varchar2(3 char) default 'INR' not null
        annotations (
            display_label 'Currency Code',
            description 'Currency used for the count variance value.',
            semantic_type 'currency_code'
        ),
    variance_value_amount number(18,2) generated always as (
        round(abs(counted_quantity - expected_quantity) * nvl(unit_cost_amount, 0), 2)
    ) virtual
        annotations (
            display_label 'Variance Value Amount',
            description 'Estimated financial value of the count variance.',
            display_in_form 'false',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    ...
    tolerance_quantity number(18,4)
        annotations (
            display_label 'Tolerance Quantity',
            description 'Quantity tolerance applied to the line when deciding whether the difference needs investigation.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    tolerance_percent number(5,2)
        annotations (
            display_label 'Tolerance Percent',
            description 'Percentage tolerance applied to the line when deciding whether the difference needs investigation.',
            format_mask 'FM990D00',
            semantic_type 'percentage'
        ),
    within_tolerance_flag boolean default false not null
        annotations (
            display_label 'Within Tolerance Flag',
            description 'Indicates whether the count difference is within the agreed tolerance.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    recount_required boolean default false not null
        annotations (
            display_label 'Recount Required',
            description 'Indicates whether the line needs a recount.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    recount_quantity number(18,4)
        annotations (
            display_label 'Recount Quantity',
            description 'Quantity found on recount where a recount was performed.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    line_status_code varchar2(20 char) default 'PENDING' not null
        annotations (
            display_label 'Line Status',
            description 'Current business status of the count line.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include PENDING, COUNTED, RECOUNTED, RESOLVED, CANCELLED.'
        ),
    first_counted_by varchar2(128 char)
        annotations (
            display_label 'First Counted By',
            description 'User who performed the first count.',
            semantic_type 'user_name'
        ),
    ...
    reviewed_by varchar2(128 char)
        annotations (
            display_label 'Reviewed By',
            description 'User who reviewed or resolved the count line.',
            semantic_type 'user_name'
        ),
    reviewed_at timestamp with time zone
        annotations (
    ...
    resolution_notes varchar2(500 char)
        annotations (
            display_label 'Resolution Notes',
            description 'Business notes explaining how the count difference was resolved.',
            semantic_type 'comment'
        ),
    ...
    constraint ck_scm_stock_count_lines_04 check (recount_quantity is null or recount_quantity >= 0),
    constraint ck_scm_stock_count_lines_05 check (line_status_code in ('PENDING', 'COUNTED', 'RECOUNTED', 'RESOLVED', 'CANCELLED')),
    constraint ck_scm_stock_count_lines_06 check (tolerance_quantity is null or tolerance_quantity >= 0),
    constraint ck_scm_stock_count_lines_07 check (
        tolerance_percent is null
        or (tolerance_percent >= 0 and tolerance_percent <= 100)
    ),
    constraint ck_scm_stock_count_lines_08 check (unit_cost_amount is null or unit_cost_amount >= 0)
) annotations (
...
create table scm_pick_waves (
    pick_wave_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Pick Wave ID',
            description 'Surrogate key for the pick wave or batch.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    wave_number varchar2(30 char) not null
        annotations (
            display_label 'Wave Number',
            description 'Business reference used to identify the pick wave.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'reference_number'
        ),
    warehouse_id number not null
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse that owns the pick wave.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    wave_type_code varchar2(20 char) default 'WAVE' not null
        annotations (
            display_label 'Wave Type Code',
            description 'Business type of the pick grouping.',
            semantic_type 'type',
            ai_context 'Valid values include WAVE, BATCH.'
        ),
    wave_status_code varchar2(20 char) default 'PLANNED' not null
        annotations (
            display_label 'Wave Status Code',
            description 'Current business status of the pick wave.',
            semantic_type 'status',
            ai_context 'Valid values include PLANNED, RELEASED, IN_PROGRESS, COMPLETED, CANCELLED.'
        ),
    priority_code varchar2(20 char) default 'MEDIUM' not null
        annotations (
            display_label 'Priority Code',
            description 'Business priority of the wave.',
            semantic_type 'priority',
            ai_context 'Valid values include LOW, MEDIUM, HIGH, CRITICAL.'
        ),
    release_at timestamp with time zone
        annotations (
            display_label 'Release At',
            description 'Date and time when the wave was released to the floor.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    started_at timestamp with time zone
        annotations (
            display_label 'Started At',
            description 'Date and time when the wave started.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    completed_at timestamp with time zone
        annotations (
            display_label 'Completed At',
            description 'Date and time when the wave completed.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    released_by varchar2(128 char)
        annotations (
            display_label 'Released By',
            description 'User who released the wave.',
            semantic_type 'user_name'
        ),
    carrier_partner_id number
        annotations (
            display_label 'Carrier Partner ID',
            description 'Carrier linked to the wave where picks are grouped by carrier run.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    wave_notes varchar2(1000 char)
        annotations (
            display_label 'Wave Notes',
            description 'Business notes for the pick wave.',
            semantic_type 'comment'
        ),
    ...
    constraint fk_scm_pick_waves_02 foreign key (carrier_partner_id)
        references scm_business_partners (business_partner_id),
    constraint ck_scm_pick_waves_01 check (wave_type_code in ('WAVE', 'BATCH')),
    constraint ck_scm_pick_waves_02 check (wave_status_code in ('PLANNED', 'RELEASED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED')),
    constraint ck_scm_pick_waves_03 check (priority_code in ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL')),
    constraint ck_scm_pick_waves_04 check (
        completed_at is null
        or started_at is null
        or completed_at >= started_at
    )
) annotations (
    display_label 'Pick Waves',
    description 'Stores records for pick waves and batches.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_operational_tasks (
    operational_task_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Operational Task ID',
            description 'Surrogate key for the operational task.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    task_number varchar2(30 char) not null
        annotations (
            display_label 'Task Number',
            description 'Business reference used to identify the task.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'reference_number'
        ),
    task_type_code varchar2(30 char) not null
        annotations (
            display_label 'Task Type Code',
            description 'Business type of warehouse task.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include PUTAWAY, REPLENISHMENT, PICK, PACK, DISPATCH_CHECK, COUNT, RECEIPT_REVIEW, EXCEPTION_REVIEW, QUARANTINE_REVIEW, TRANSFER_RECEIPT.'
        ),
    warehouse_id number not null
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse responsible for the task.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    from_location_id number
        annotations (
            display_label 'From Location ID',
            description 'Starting location for the task where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    to_location_id number
        annotations (
            display_label 'To Location ID',
            description 'Destination location for the task where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    item_id number
        annotations (
            display_label 'Item ID',
            description 'Item linked to the task where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    inventory_lot_id number
        annotations (
            display_label 'Inventory Lot ID',
            description 'Lot linked to the task where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    source_document_type varchar2(30 char)
        annotations (
            display_label 'Source Document Type',
            description 'Business document type that created the task.',
            semantic_type 'type'
        ),
    source_document_number varchar2(50 char)
        annotations (
            display_label 'Source Document Number',
            description 'Business document number that created the task.',
            semantic_type 'reference_number'
        ),
    source_line_number number(6)
        annotations (
            display_label 'Source Line Number',
            description 'Source line number linked to the task where relevant.',
            semantic_type 'sequence'
        ),
    task_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            display_label 'Task Status Code',
            description 'Current business status of the task.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, ASSIGNED, IN_PROGRESS, COMPLETED, CANCELLED, ON_HOLD.'
        ),
    priority_code varchar2(20 char) default 'MEDIUM' not null
        annotations (
            display_label 'Priority Code',
            description 'Business priority of the task.',
            semantic_type 'priority',
            ai_context 'Valid values include LOW, MEDIUM, HIGH, CRITICAL.'
        ),
    task_quantity number(18,4)
        annotations (
            display_label 'Task Quantity',
            description 'Quantity that the task covers where relevant.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    pick_wave_id number
        annotations (
            display_label 'Pick Wave ID',
            description 'Pick wave or batch that groups the task where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    assigned_role_id number
        annotations (
            display_label 'Assigned Role ID',
            description 'Role currently responsible for the task.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    assigned_user_id number
        annotations (
            ...
            description 'User currently responsible for the task where assigned to a person.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    assigned_by_user_id number
    ...
    started_at timestamp with time zone
        annotations (
            display_label 'Started At',
            description 'Date and time when work started.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    completed_at timestamp with time zone
        annotations (
            display_label 'Completed At',
            description 'Date and time when work completed.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    completion_code varchar2(30 char)
        annotations (
            display_label 'Completion Code',
            description 'Business outcome recorded when the task completed.',
            semantic_type 'resolution'
        ),
    task_notes varchar2(1000 char)
        annotations (
            display_label 'Task Notes',
            description 'Business notes for the task.',
            semantic_type 'comment'
        ),
    ...
    constraint fk_scm_operational_tasks_08 foreign key (assigned_by_user_id)
        references scm_application_users (application_user_id),
    constraint ck_scm_operational_tasks_01 check (
        task_type_code in (
            'PUTAWAY',
            'REPLENISHMENT',
            'PICK',
    ...
    constraint ck_scm_operational_tasks_02 check (task_status_code in ('OPEN', 'ASSIGNED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'ON_HOLD')),
    constraint ck_scm_operational_tasks_03 check (priority_code in ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL')),
    constraint ck_scm_operational_tasks_04 check (task_quantity is null or task_quantity > 0),
    constraint ck_scm_operational_tasks_05 check (source_line_number is null or source_line_number > 0)
) annotations (
    display_label 'Operational Tasks',
    description 'Stores records for operational tasks.',
    application_module 'Inventory and Warehouse Management'
);
...
create table scm_operational_exceptions (
    ...
    exception_type_code varchar2(30 char) not null
        annotations (
            ...
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include RECEIPT_DAMAGE, WRONG_ITEM_RECEIVED, RECEIPT_SHORTAGE, RECEIPT_OVERAGE, WRONG_LOCATION, PICK_NOT_FOUND, PICK_DAMAGE, DISPATCH_DISCREPANCY, POST_DISPATCH_DISCREPANCY, RETURN_NOT_REUSABLE, EXPIRED_STOCK, QUARANTINE_USE_ATTEMPT, ROTATION_EXCEPTION, OTHER.'
        ),
    warehouse_id number not null
        annotations (
            display_label 'Warehouse ID',
    ...
    source_document_type varchar2(30 char)
        annotations (
            display_label 'Source Document Type',
            description 'Business document type that the exception relates to.',
            semantic_type 'type'
        ),
    source_document_number varchar2(50 char)
        annotations (
            display_label 'Source Document Number',
            description 'Business document number that the exception relates to.',
            semantic_type 'reference_number'
        ),
    source_line_number number(6)
        annotations (
            display_label 'Source Line Number',
            description 'Source line number linked to the exception where relevant.',
            semantic_type 'sequence'
        ),
    ...
    exception_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            display_label 'Exception Status Code',
            description 'Current business status of the exception.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, UNDER_REVIEW, PENDING_APPROVAL, RESOLVED, CLOSED, CANCELLED.'
        ),
    ...
    requires_approval_flag boolean default false not null
        annotations (
            display_label 'Requires Approval Flag',
            description 'Indicates whether the exception needs formal approval before closure.',
            search_facet 'distinct-list',
            semantic_type 'flag'
        ),
    approval_policy_id number
        annotations (
            display_label 'Approval Policy ID',
            description 'Approval policy applied to the exception where approval is required.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    ...
    assigned_role_id number
        annotations (
            display_label 'Assigned Role ID',
            description 'Role responsible for reviewing the exception.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    assigned_user_id number
    ...
    resolved_at timestamp with time zone
        annotations (
            display_label 'Resolved At',
            description 'Date and time when the exception was resolved.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    resolved_by_user_id number
        annotations (
            display_label 'Resolved By User ID',
    ...
    constraint fk_scm_operational_exc_09 foreign key (resolved_by_user_id)
        references scm_application_users (application_user_id),
    constraint ck_scm_operational_exc_01 check (
    ...
    constraint ck_scm_operational_exc_02 check (severity_code in ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL')),
    constraint ck_scm_operational_exc_03 check (exception_status_code in ('OPEN', 'UNDER_REVIEW', 'PENDING_APPROVAL', 'RESOLVED', 'CLOSED', 'CANCELLED')),
    constraint ck_scm_operational_exc_04 check (affected_quantity is null or affected_quantity >= 0),
    constraint ck_scm_operational_exc_05 check (source_line_number is null or source_line_number > 0),
    constraint ck_scm_operational_exc_06 check (requires_approval_flag = false or approval_policy_id is not null)
) annotations (
...
create table scm_approval_policy_steps (
    approval_policy_step_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Approval Policy Step ID',
            description 'Surrogate key for the approval policy step.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    approval_policy_id number not null
        annotations (
            display_label 'Approval Policy ID',
            description 'Approval policy that owns the step.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    step_sequence_number number(4) not null
        annotations (
            display_label 'Step Sequence Number',
            description 'Execution sequence of the step within the approval policy.',
            value_required 'true',
            semantic_type 'sequence'
        ),
    step_name varchar2(100 char) not null
        annotations (
            display_label 'Step Name',
            description 'Business name of the approval step.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'name'
        ),
    reviewer_role_id number not null
        annotations (
            display_label 'Reviewer Role ID',
            description 'Role that should normally review the step.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    fallback_role_id number
        annotations (
            display_label 'Fallback Role ID',
            description 'Fallback role used when the normal reviewer is unavailable.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    escalation_role_id number
        annotations (
            display_label 'Escalation Role ID',
            description 'Role used when the step must be escalated after the SLA expires.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    due_within_hours number(6)
        annotations (
            display_label 'Due Within Hours',
            description 'Target SLA in hours for the step.',
            semantic_type 'duration_hours'
        ),
    escalation_after_hours number(6)
        annotations (
            display_label 'Escalation After Hours',
            description 'Hours after which the step should escalate if there is no decision.',
            semantic_type 'duration_hours'
        ),
    approval_authority_level number(4)
        annotations (
            display_label 'Approval Authority Level',
            description 'Authority level required to act on the step where this differs from the policy default.',
            semantic_type 'ranking'
        ),
    is_final_step boolean default false not null
        annotations (
            display_label 'Is Final Step',
            description 'Indicates whether approval of this step completes the approval chain.',
            semantic_type 'flag'
        ),
    step_status_code varchar2(20 char) default 'ACTIVE' not null
        annotations (
            display_label 'Step Status Code',
            description 'Current business status of the approval policy step.',
            semantic_type 'status',
            ai_context 'Valid values include ACTIVE, INACTIVE.'
        ),
    step_notes varchar2(500 char)
        annotations (
            display_label 'Step Notes',
            description 'Business notes for the approval policy step.',
            semantic_type 'comment'
        ),
    constraint pk_scm_approval_policy_steps primary key (approval_policy_step_id),
    constraint uq_scm_approval_policy_steps_01 unique (approval_policy_id, step_sequence_number),
    ...
    constraint ck_scm_approval_policy_steps_02 check (due_within_hours is null or due_within_hours > 0),
    constraint ck_scm_approval_policy_steps_03 check (escalation_after_hours is null or escalation_after_hours > 0),
    constraint ck_scm_approval_policy_steps_04 check (approval_authority_level is null or approval_authority_level > 0),
    constraint ck_scm_approval_policy_steps_05 check (step_status_code in ('ACTIVE', 'INACTIVE')),
    constraint ck_scm_approval_policy_steps_06 check (
        escalation_after_hours is null
        or due_within_hours is null
        or escalation_after_hours >= due_within_hours
    )
) annotations (
    display_label 'Approval Policy Steps',
    description 'Stores records for approval policy routing steps.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_approval_requests (
    approval_request_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Approval Request ID',
            description 'Surrogate key for the approval request.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    approval_request_number varchar2(30 char) not null
        annotations (
            display_label 'Approval Request Number',
            description 'Business reference used to identify the approval request.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'reference_number'
        ),
    approval_subject_code varchar2(30 char) not null
        annotations (
            display_label 'Approval Subject Code',
            description 'Business object under approval.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include STOCK_TRANSFER, STOCK_ADJUSTMENT, QUARANTINE_CASE, OPERATIONAL_EXCEPTION, INBOUND_RECEIPT_LINE, STOCK_COUNT_LINE, OUTBOUND_ORDER_LINE.'
        ),
    approval_policy_id number not null
        annotations (
            display_label 'Approval Policy ID',
            description 'Approval policy governing the request.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    current_step_sequence_number number(4) default 1 not null
        annotations (
            display_label 'Current Step Sequence Number',
            description 'Current approval step sequence for the request.',
            semantic_type 'sequence'
        ),
    current_reviewer_role_id number
        annotations (
            display_label 'Current Reviewer Role ID',
            description 'Role currently expected to act on the request.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    fallback_reviewer_role_id number
        annotations (
            display_label 'Fallback Reviewer Role ID',
            description 'Fallback role configured for the current request step where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    escalated_reviewer_role_id number
        annotations (
            display_label 'Escalated Reviewer Role ID',
            description 'Role that owns the request after escalation where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    approval_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            display_label 'Approval Status Code',
            description 'Current business status of the approval request.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, APPROVED, REJECTED, RETURNED, ESCALATED, CANCELLED, CLOSED.'
        ),
    requested_by varchar2(128 char) not null
        annotations (
            display_label 'Requested By',
    ...
    requested_at timestamp with time zone default systimestamp not null
        annotations (
            display_label 'Requested At',
            description 'Date and time when the approval request was submitted.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    due_at timestamp with time zone
        annotations (
            display_label 'Due At',
            description 'Date and time when the current approval step is due.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    escalated_at timestamp with time zone
        annotations (
            display_label 'Escalated At',
            description 'Date and time when the request was escalated.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    completed_at timestamp with time zone
        annotations (
            display_label 'Completed At',
            description 'Date and time when the request reached a terminal outcome.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    stock_transfer_id number
        annotations (
            display_label 'Stock Transfer ID',
            description 'Stock transfer under approval where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    stock_adjustment_id number
        annotations (
            display_label 'Stock Adjustment ID',
            description 'Stock adjustment under approval where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    quarantine_case_id number
        annotations (
            display_label 'Quarantine Case ID',
            description 'Quarantine case under approval where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    operational_exception_id number
        annotations (
            display_label 'Operational Exception ID',
            description 'Operational exception under approval where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    inbound_receipt_line_id number
        annotations (
            display_label 'Inbound Receipt Line ID',
            description 'Inbound receipt line under approval where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    stock_count_line_id number
        annotations (
            display_label 'Stock Count Line ID',
            description 'Stock count line under approval where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    outbound_order_line_id number
        annotations (
            display_label 'Outbound Order Line ID',
            description 'Outbound order line under approval where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    request_reason_code varchar2(30 char)
        annotations (
            display_label 'Request Reason Code',
            description 'Business reason code recorded when the approval request was submitted.',
            semantic_type 'reason_code'
        ),
    request_notes varchar2(1000 char)
        annotations (
            display_label 'Request Notes',
            description 'Business notes for the approval request.',
            semantic_type 'comment'
        ),
    ...
    constraint ck_scm_approval_requests_01 check (
        approval_subject_code in (
            'STOCK_TRANSFER',
            'STOCK_ADJUSTMENT',
            'QUARANTINE_CASE',
    ...
    constraint ck_scm_approval_requests_03 check (
        approval_status_code in ('OPEN', 'APPROVED', 'REJECTED', 'RETURNED', 'ESCALATED', 'CANCELLED', 'CLOSED')
    ),
    constraint ck_scm_approval_requests_04 check (
        due_at is null
        or due_at >= requested_at
    ),
    constraint ck_scm_approval_requests_05 check (
        completed_at is null
        or completed_at >= requested_at
    ),
    constraint ck_scm_approval_requests_06 check (
        (case when stock_transfer_id is null then 0 else 1 end) +
        (case when stock_adjustment_id is null then 0 else 1 end) +
        (case when quarantine_case_id is null then 0 else 1 end) +
        (case when operational_exception_id is null then 0 else 1 end) +
        (case when inbound_receipt_line_id is null then 0 else 1 end) +
        (case when stock_count_line_id is null then 0 else 1 end) +
        (case when outbound_order_line_id is null then 0 else 1 end) = 1
    ),
    constraint ck_scm_approval_requests_07 check (
        (approval_subject_code = 'STOCK_TRANSFER' and stock_transfer_id is not null and stock_adjustment_id is null and quarantine_case_id is null and operational_exception_id is null and inbound_receipt_line_id is null and stock_count_line_id is null and outbound_order_line_id is null)
        or (approval_subject_code = 'STOCK_ADJUSTMENT' and stock_transfer_id is null and stock_adjustment_id is not null and quarantine_case_id is null and operational_exception_id is null and inbound_receipt_line_id is null and stock_count_line_id is null and outbound_order_line_id is null)
        or (approval_subject_code = 'QUARANTINE_CASE' and stock_transfer_id is null and stock_adjustment_id is null and quarantine_case_id is not null and operational_exception_id is null and inbound_receipt_line_id is null and stock_count_line_id is null and outbound_order_line_id is null)
        or (approval_subject_code = 'OPERATIONAL_EXCEPTION' and stock_transfer_id is null and stock_adjustment_id is null and quarantine_case_id is null and operational_exception_id is not null and inbound_receipt_line_id is null and stock_count_line_id is null and outbound_order_line_id is null)
        or (approval_subject_code = 'INBOUND_RECEIPT_LINE' and stock_transfer_id is null and stock_adjustment_id is null and quarantine_case_id is null and operational_exception_id is null and inbound_receipt_line_id is not null and stock_count_line_id is null and outbound_order_line_id is null)
        or (approval_subject_code = 'STOCK_COUNT_LINE' and stock_transfer_id is null and stock_adjustment_id is null and quarantine_case_id is null and operational_exception_id is null and inbound_receipt_line_id is null and stock_count_line_id is not null and outbound_order_line_id is null)
        or (approval_subject_code = 'OUTBOUND_ORDER_LINE' and stock_transfer_id is null and stock_adjustment_id is null and quarantine_case_id is null and operational_exception_id is null and inbound_receipt_line_id is null and stock_count_line_id is null and outbound_order_line_id is not null)
    )
) annotations (
    display_label 'Approval Requests',
    description 'Stores records for active and completed approval requests.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_approval_actions (
    approval_action_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Approval Action ID',
            description 'Surrogate key for the approval action.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    approval_request_id number not null
        annotations (
            display_label 'Approval Request ID',
            description 'Approval request that this action belongs to.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    approval_sequence_number number(4) default 1 not null
        annotations (
            display_label 'Approval Sequence Number',
            description 'Sequence of the approval step for the approval request.',
            semantic_type 'sequence'
        ),
    approval_step_name varchar2(100 char) not null
        annotations (
            display_label 'Approval Step Name',
            description 'Business name of the approval step.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'name'
    ...
    assigned_role_id number
        annotations (
            display_label 'Assigned Role ID',
            description 'Role responsible for the step when the action was taken.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    decision_code varchar2(20 char) not null
        annotations (
            display_label 'Decision Code',
            description 'Approval decision taken at the step.',
            value_required 'true',
            semantic_type 'decision',
            ai_context 'Valid values include SUBMITTED, APPROVED, REJECTED, RETURNED, ESCALATED, REASSIGNED.'
        ),
    decision_by varchar2(128 char) not null
        annotations (
            display_label 'Decision By',
            description 'User who took the approval decision.',
            value_required 'true',
            semantic_type 'user_name'
        ),
    decision_at timestamp with time zone default systimestamp not null
        annotations (
            display_label 'Decision At',
            description 'Date and time when the approval decision was recorded.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    decision_comment varchar2(1000 char)
        annotations (
            display_label 'Decision Comment',
            description 'Business comment recorded with the approval decision.',
            semantic_type 'comment'
        ),
    ...
    constraint ck_scm_approval_actions_02 check (
        decision_code in ('SUBMITTED', 'APPROVED', 'REJECTED', 'RETURNED', 'ESCALATED', 'REASSIGNED')
    )
) annotations (
    display_label 'Approval Actions',
    description 'Stores records for approval actions.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_alert_rules (
    alert_rule_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Alert Rule ID',
            description 'Surrogate key for the alert rule.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    rule_code varchar2(30 char) not null
        annotations (
            display_label 'Rule Code',
            description 'Business code used to identify the alert rule.',
            value_required 'true',
            semantic_type 'code'
        ),
    rule_name varchar2(150 char) not null
        annotations (
            display_label 'Rule Name',
            description 'Business name of the alert rule.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'name'
        ),
    warehouse_id number
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse where the rule applies where the rule is site specific.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    process_area_code varchar2(20 char)
        annotations (
            display_label 'Process Area Code',
            description 'Warehouse process area covered by the rule.',
            semantic_type 'classification',
            ai_context 'Valid values include INBOUND, PUTAWAY, INVENTORY, REPLENISHMENT, OUTBOUND, RETURNS, QUARANTINE, COUNTING, APPROVALS, REPORTING, ADMINISTRATION.'
        ),
    alert_type_code varchar2(30 char) not null
        annotations (
            display_label 'Alert Type Code',
            description 'Business alert type raised by the rule.',
            value_required 'true',
            semantic_type 'classification'
        ),
    alert_subject_code varchar2(30 char) not null
        annotations (
            display_label 'Alert Subject Code',
            description 'Business subject area that the alert rule watches.',
            value_required 'true',
            semantic_type 'classification',
            ai_context 'Valid values include REPLENISHMENT_ALERT, STOCK_TRANSFER, INBOUND_RECEIPT, OPERATIONAL_TASK, STOCK_COUNT, APPROVAL_REQUEST, OPERATIONAL_EXCEPTION, QUARANTINE_CASE, OUTBOUND_ORDER.'
        ),
    threshold_quantity number(18,4)
        annotations (
            display_label 'Threshold Quantity',
            description 'Quantity threshold used by the rule where relevant.',
            format_mask 'FM999G999G999G990D0000',
            semantic_type 'quantity'
        ),
    threshold_value_amount number(18,2)
        annotations (
            display_label 'Threshold Value Amount',
            description 'Value threshold used by the rule where relevant.',
            format_mask 'FM999G999G999G990D00',
            semantic_type 'amount'
        ),
    age_threshold_hours number(6)
        annotations (
            display_label 'Age Threshold Hours',
            description 'Age threshold in hours used by the rule where relevant.',
            semantic_type 'duration_hours'
        ),
    recipient_role_id number
        annotations (
            display_label 'Recipient Role ID',
            description 'Role that should receive the alert when it is raised.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    escalation_role_id number
        annotations (
            display_label 'Escalation Role ID',
            description 'Role that should receive the alert when it is escalated.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    delivery_method_code varchar2(20 char) default 'IN_APP' not null
        annotations (
            display_label 'Delivery Method Code',
            description 'Delivery method used by the alert rule.',
            semantic_type 'classification',
            ai_context 'Valid values include IN_APP, EMAIL, SMS.'
        ),
    priority_code varchar2(20 char) default 'MEDIUM' not null
        annotations (
            display_label 'Priority Code',
            description 'Priority assigned when the rule raises an alert.',
            semantic_type 'priority',
            ai_context 'Valid values include LOW, MEDIUM, HIGH, CRITICAL.'
        ),
    is_active boolean default true not null
        annotations (
            display_label 'Is Active',
            description 'Indicates whether the alert rule is active.',
            semantic_type 'flag'
        ),
    rule_notes varchar2(500 char)
        annotations (
            display_label 'Rule Notes',
            description 'Business notes for the alert rule.',
            semantic_type 'comment'
        ),
    ...
    constraint fk_scm_alert_rules_03 foreign key (escalation_role_id)
        references scm_user_roles (user_role_id),
    constraint ck_scm_alert_rules_01 check (
        process_area_code in ('INBOUND', 'PUTAWAY', 'INVENTORY', 'REPLENISHMENT', 'OUTBOUND', 'RETURNS', 'QUARANTINE', 'COUNTING', 'APPROVALS', 'REPORTING', 'ADMINISTRATION')
        or process_area_code is null
    ),
    constraint ck_scm_alert_rules_02 check (
        alert_subject_code in ('REPLENISHMENT_ALERT', 'STOCK_TRANSFER', 'INBOUND_RECEIPT', 'OPERATIONAL_TASK', 'STOCK_COUNT', 'APPROVAL_REQUEST', 'OPERATIONAL_EXCEPTION', 'QUARANTINE_CASE', 'OUTBOUND_ORDER')
    ),
    constraint ck_scm_alert_rules_03 check (threshold_quantity is null or threshold_quantity >= 0),
    constraint ck_scm_alert_rules_04 check (threshold_value_amount is null or threshold_value_amount >= 0),
    constraint ck_scm_alert_rules_05 check (age_threshold_hours is null or age_threshold_hours > 0),
    constraint ck_scm_alert_rules_06 check (delivery_method_code in ('IN_APP', 'EMAIL', 'SMS')),
    constraint ck_scm_alert_rules_07 check (priority_code in ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL'))
) annotations (
    display_label 'Alert Rules',
    description 'Stores records for configurable alert rules.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_alerts (
    alert_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Alert ID',
            description 'Surrogate key for the alert.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    alert_number varchar2(30 char) not null
        annotations (
            display_label 'Alert Number',
            description 'Business reference used to identify the alert.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'reference_number'
        ),
    alert_rule_id number
        annotations (
            display_label 'Alert Rule ID',
            description 'Alert rule that raised the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    warehouse_id number not null
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse responsible for the alert.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    alert_type_code varchar2(30 char) not null
        annotations (
            display_label 'Alert Type Code',
            description 'Business alert type for the alert instance.',
            value_required 'true',
            semantic_type 'classification'
        ),
    alert_subject_code varchar2(30 char) not null
        annotations (
            display_label 'Alert Subject Code',
            description 'Business subject area that triggered the alert.',
            value_required 'true',
            semantic_type 'classification',
            ai_context 'Valid values include REPLENISHMENT_ALERT, STOCK_TRANSFER, INBOUND_RECEIPT, OPERATIONAL_TASK, STOCK_COUNT, APPROVAL_REQUEST, OPERATIONAL_EXCEPTION, QUARANTINE_CASE, OUTBOUND_ORDER.'
        ),
    alert_status_code varchar2(20 char) default 'OPEN' not null
        annotations (
            display_label 'Alert Status Code',
            description 'Current business status of the alert.',
            search_facet 'distinct-list',
            semantic_type 'status',
            ai_context 'Valid values include OPEN, ACKNOWLEDGED, IN_PROGRESS, RESOLVED, CLOSED, SUPPRESSED.'
        ),
    priority_code varchar2(20 char) default 'MEDIUM' not null
        annotations (
            display_label 'Priority Code',
            description 'Priority of the alert instance.',
            semantic_type 'priority',
            ai_context 'Valid values include LOW, MEDIUM, HIGH, CRITICAL.'
        ),
    replenishment_alert_id number
        annotations (
            display_label 'Replenishment Alert ID',
            description 'Replenishment alert linked to the generic alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    stock_transfer_id number
        annotations (
            display_label 'Stock Transfer ID',
            description 'Stock transfer linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    inbound_receipt_id number
        annotations (
            display_label 'Inbound Receipt ID',
            description 'Inbound receipt linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    operational_task_id number
        annotations (
            display_label 'Operational Task ID',
            description 'Operational task linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    stock_count_id number
        annotations (
            display_label 'Stock Count ID',
            description 'Stock count linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    approval_request_id number
        annotations (
            display_label 'Approval Request ID',
            description 'Approval request linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    operational_exception_id number
        annotations (
            display_label 'Operational Exception ID',
            description 'Operational exception linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    quarantine_case_id number
        annotations (
            display_label 'Quarantine Case ID',
            description 'Quarantine case linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    outbound_order_id number
        annotations (
            display_label 'Outbound Order ID',
            description 'Outbound order linked to the alert where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    raised_at timestamp with time zone default systimestamp not null
    ...
    due_at timestamp with time zone
        annotations (
            display_label 'Due At',
            description 'Date and time when action on the alert is due.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    acknowledged_at timestamp with time zone
        annotations (
            display_label 'Acknowledged At',
            description 'Date and time when the alert was acknowledged.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    acknowledged_by_user_id number
        annotations (
            display_label 'Acknowledged By User ID',
    ...
    alert_message varchar2(500 char) not null
        annotations (
            display_label 'Alert Message',
            description 'User-facing message shown for the alert.',
            value_required 'true',
            semantic_type 'description'
        ),
    alert_notes varchar2(1000 char)
        annotations (
            display_label 'Alert Notes',
            description 'Business notes for the alert.',
            semantic_type 'comment'
        ),
    ...
    constraint fk_scm_alerts_14 foreign key (resolved_by_user_id)
        references scm_application_users (application_user_id),
    constraint ck_scm_alerts_01 check (
        alert_subject_code in ('REPLENISHMENT_ALERT', 'STOCK_TRANSFER', 'INBOUND_RECEIPT', 'OPERATIONAL_TASK', 'STOCK_COUNT', 'APPROVAL_REQUEST', 'OPERATIONAL_EXCEPTION', 'QUARANTINE_CASE', 'OUTBOUND_ORDER')
    ),
    constraint ck_scm_alerts_02 check (
        alert_status_code in ('OPEN', 'ACKNOWLEDGED', 'IN_PROGRESS', 'RESOLVED', 'CLOSED', 'SUPPRESSED')
    ),
    constraint ck_scm_alerts_03 check (priority_code in ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL')),
    constraint ck_scm_alerts_04 check (
        (case when replenishment_alert_id is null then 0 else 1 end) +
        (case when stock_transfer_id is null then 0 else 1 end) +
        (case when inbound_receipt_id is null then 0 else 1 end) +
        (case when operational_task_id is null then 0 else 1 end) +
        (case when stock_count_id is null then 0 else 1 end) +
        (case when approval_request_id is null then 0 else 1 end) +
        (case when operational_exception_id is null then 0 else 1 end) +
        (case when quarantine_case_id is null then 0 else 1 end) +
        (case when outbound_order_id is null then 0 else 1 end) >= 1
    )
) annotations (
    display_label 'Alerts',
    description 'Stores records for operational alerts and inbox items.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_alert_notifications (
    alert_notification_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Alert Notification ID',
            description 'Surrogate key for the alert notification.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    alert_id number not null
        annotations (
            display_label 'Alert ID',
            description 'Alert that the notification belongs to.',
            value_required 'true',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    recipient_role_id number
        annotations (
            display_label 'Recipient Role ID',
            description 'Role recipient for the alert notification where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    recipient_user_id number
        annotations (
            display_label 'Recipient User ID',
            description 'User recipient for the alert notification where relevant.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    delivery_method_code varchar2(20 char) default 'IN_APP' not null
        annotations (
            display_label 'Delivery Method Code',
            description 'Delivery method used for the alert notification.',
            semantic_type 'classification',
            ai_context 'Valid values include IN_APP, EMAIL, SMS.'
        ),
    notification_status_code varchar2(20 char) default 'PENDING' not null
        annotations (
            display_label 'Notification Status Code',
            description 'Current business status of the alert notification.',
            semantic_type 'status',
            ai_context 'Valid values include PENDING, SENT, READ, ACKNOWLEDGED, FAILED.'
        ),
    notification_address varchar2(200 char)
        annotations (
            display_label 'Notification Address',
            description 'Target notification address used for delivery where relevant.',
            semantic_type 'contact_point'
        ),
    sent_at timestamp with time zone
        annotations (
            display_label 'Sent At',
    ...
    acknowledged_at timestamp with time zone
        annotations (
            display_label 'Acknowledged At',
            description 'Date and time when the notification was acknowledged.',
            format_mask 'DD-MON-YYYY HH24:MI TZH:TZM',
            semantic_type 'datetime'
        ),
    notification_notes varchar2(500 char)
        annotations (
            display_label 'Notification Notes',
            description 'Business notes for the alert notification.',
            semantic_type 'comment'
        ),
    ...
    constraint fk_scm_alert_notifications_03 foreign key (recipient_user_id)
        references scm_application_users (application_user_id),
    constraint ck_scm_alert_notifications_01 check (
        delivery_method_code in ('IN_APP', 'EMAIL', 'SMS')
    ),
    constraint ck_scm_alert_notifications_02 check (
        notification_status_code in ('PENDING', 'SENT', 'READ', 'ACKNOWLEDGED', 'FAILED')
    ),
    constraint ck_scm_alert_notifications_03 check (
        recipient_role_id is not null
        or recipient_user_id is not null
    )
) annotations (
    display_label 'Alert Notifications',
    description 'Stores records for alert notification delivery and inbox acknowledgement.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_record_notes (
    record_note_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Record Note ID',
            description 'Surrogate key for the generic record note.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    record_type_code varchar2(30 char) not null
        annotations (
            display_label 'Record Type Code',
            description 'Business entity type that the note belongs to.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include BUSINESS_PARTNER, PARTNER_SITE, WAREHOUSE, WAREHOUSE_AREA, STORAGE_LOCATION, ITEM, ITEM_SERIAL, INVENTORY_LOT, INBOUND_RECEIPT, INBOUND_RECEIPT_LINE, STOCK_TRANSFER, STOCK_TRANSFER_LINE, OUTBOUND_ORDER, OUTBOUND_ORDER_LINE, RETURN, RETURN_LINE, QUARANTINE_CASE, STOCK_ADJUSTMENT, STOCK_ADJUSTMENT_LINE, STOCK_COUNT, STOCK_COUNT_LINE, OPERATIONAL_TASK, OPERATIONAL_EXCEPTION, APPROVAL_REQUEST, ALERT.'
        ),
    record_id number not null
        annotations (
            display_label 'Record ID',
            description 'Identifier of the related business record.',
            value_required 'true',
            semantic_type 'identifier'
        ),
    warehouse_id number
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse linked to the note where the record is warehouse specific.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    note_type_code varchar2(20 char) default 'GENERAL' not null
        annotations (
            display_label 'Note Type Code',
            description 'Business note type used to group the note.',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include GENERAL, RECEIVING, QUALITY, DISPATCH, APPROVAL, COUNT, ADJUSTMENT, CUSTOMER, INTERNAL.'
        ),
    note_title varchar2(150 char)
        annotations (
            display_label 'Note Title',
            description 'Short heading used to identify the note.',
            semantic_type 'title'
        ),
    note_text varchar2(2000 char) not null
        annotations (
            display_label 'Note Text',
            description 'Business note text shown on the workspace.',
            value_required 'true',
            semantic_type 'comment'
        ),
    visibility_code varchar2(20 char) default 'INTERNAL' not null
        annotations (
            display_label 'Visibility Code',
            description 'Visibility level for the note.',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include INTERNAL, SHARED, CUSTOMER_VISIBLE.'
        ),
    pinned_flag boolean default false not null
        annotations (
            display_label 'Pinned Flag',
            description 'Indicates whether the note should stay visible at the top of the workspace.',
            semantic_type 'flag'
        ),
    created_at timestamp with time zone default systimestamp not null
        annotations (
    ...
    created_by varchar2(128 char) default sys_context('USERENV','SESSION_USER') not null
        annotations (
            display_label 'Created By',
            description 'Database user that created the note.',
            display_in_form 'false',
            semantic_type 'user_name'
        ),
    ...
    constraint ck_scm_record_notes_02 check (
        record_type_code in (
            'BUSINESS_PARTNER',
            'PARTNER_SITE',
            'WAREHOUSE',
    ...
    constraint ck_scm_record_notes_03 check (
        note_type_code in ('GENERAL', 'RECEIVING', 'QUALITY', 'DISPATCH', 'APPROVAL', 'COUNT', 'ADJUSTMENT', 'CUSTOMER', 'INTERNAL')
    ),
    constraint ck_scm_record_notes_04 check (
        visibility_code in ('INTERNAL', 'SHARED', 'CUSTOMER_VISIBLE')
    )
) annotations (
    display_label 'Record Notes',
    description 'Stores generic notes linked to supported warehouse records.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_record_attachments (
    record_attachment_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Record Attachment ID',
            description 'Surrogate key for the generic record attachment.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    record_type_code varchar2(30 char) not null
        annotations (
            display_label 'Record Type Code',
            description 'Business entity type that the attachment belongs to.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include BUSINESS_PARTNER, PARTNER_SITE, WAREHOUSE, WAREHOUSE_AREA, STORAGE_LOCATION, ITEM, ITEM_SERIAL, INVENTORY_LOT, INBOUND_RECEIPT, INBOUND_RECEIPT_LINE, STOCK_TRANSFER, STOCK_TRANSFER_LINE, OUTBOUND_ORDER, OUTBOUND_ORDER_LINE, RETURN, RETURN_LINE, QUARANTINE_CASE, STOCK_ADJUSTMENT, STOCK_ADJUSTMENT_LINE, STOCK_COUNT, STOCK_COUNT_LINE, OPERATIONAL_TASK, OPERATIONAL_EXCEPTION, APPROVAL_REQUEST, ALERT.'
    ...
    warehouse_id number
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse linked to the attachment where the record is warehouse specific.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    attachment_category_code varchar2(30 char) default 'GENERAL' not null
        annotations (
            display_label 'Attachment Category Code',
            description 'Business category of the attachment.',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include GENERAL, RECEIPT_PROOF, DAMAGE_PHOTO, QUALITY_DOCUMENT, DISPATCH_DOCUMENT, APPROVAL_SUPPORT, COUNT_EVIDENCE.'
        ),
    ...
    mime_type varchar2(100 char) not null
        annotations (
            display_label 'MIME Type',
            description 'MIME type of the uploaded file.',
            value_required 'true',
            semantic_type 'mime_type'
        ),
    file_size_bytes number(12) not null
        annotations (
            display_label 'File Size Bytes',
            description 'File size in bytes.',
            format_mask 'FM999G999G999G990',
            semantic_type 'size_bytes'
        ),
    storage_reference varchar2(500 char) not null
        annotations (
            display_label 'Storage Reference',
            description 'Document storage reference used by the application to retrieve the file.',
            value_required 'true',
            semantic_type 'file_path'
        ),
    ...
    uploaded_by varchar2(128 char) default sys_context('USERENV','SESSION_USER') not null
        annotations (
            display_label 'Uploaded By',
            description 'Database user that uploaded the attachment.',
            display_in_form 'false',
            semantic_type 'user_name'
        ),
    is_active boolean default true not null
        annotations (
            display_label 'Is Active',
            description 'Indicates whether the attachment remains active for the record.',
            semantic_type 'flag'
        ),
    attachment_notes varchar2(500 char)
        annotations (
            display_label 'Attachment Notes',
            description 'Business notes about the attachment.',
            semantic_type 'comment'
        ),
    ...
    constraint ck_scm_record_attachments_02 check (
        record_type_code in (
            'BUSINESS_PARTNER',
            'PARTNER_SITE',
            'WAREHOUSE',
    ...
    constraint ck_scm_record_attachments_03 check (
        attachment_category_code in (
            'GENERAL',
            'RECEIPT_PROOF',
            'DAMAGE_PHOTO',
            'QUALITY_DOCUMENT',
    ...
    display_label 'Record Attachments',
    description 'Stores generic attachment metadata linked to supported warehouse records.',
    application_module 'Inventory and Warehouse Management'
);

create table scm_audit_events (
    audit_event_id number generated always as identity (start with 1 increment by 1 cache 100)
        annotations (
            display_label 'Audit Event ID',
            description 'Surrogate key for the generic audit event.',
            display_in_report 'false',
            display_in_form 'false',
            semantic_type 'identifier'
        ),
    record_type_code varchar2(30 char) not null
        annotations (
            display_label 'Record Type Code',
            description 'Business entity type that the audit event belongs to.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'classification',
            ai_context 'Valid values include BUSINESS_PARTNER, PARTNER_SITE, WAREHOUSE, WAREHOUSE_AREA, STORAGE_LOCATION, ITEM, ITEM_SERIAL, INVENTORY_LOT, INBOUND_RECEIPT, INBOUND_RECEIPT_LINE, STOCK_TRANSFER, STOCK_TRANSFER_LINE, OUTBOUND_ORDER, OUTBOUND_ORDER_LINE, RETURN, RETURN_LINE, QUARANTINE_CASE, STOCK_ADJUSTMENT, STOCK_ADJUSTMENT_LINE, STOCK_COUNT, STOCK_COUNT_LINE, OPERATIONAL_TASK, OPERATIONAL_EXCEPTION, APPROVAL_REQUEST, ALERT.'
    ...
    warehouse_id number
        annotations (
            display_label 'Warehouse ID',
            description 'Warehouse linked to the audit event where the record is warehouse specific.',
            display_as_lov 'select-list',
            semantic_type 'identifier'
        ),
    event_type_code varchar2(30 char) not null
        annotations (
            display_label 'Event Type Code',
            description 'Business event recorded in the audit history.',
            value_required 'true',
            search_facet 'distinct-list',
            semantic_type 'type',
            ai_context 'Valid values include CREATED, UPDATED, STATUS_CHANGED, RECEIVED, PUTAWAY, ALLOCATED, PICKED, DISPATCHED, COUNTED, ADJUSTED, APPROVAL_SUBMITTED, APPROVED, REJECTED, ALERT_RAISED, NOTE_ADDED, ATTACHMENT_ADDED.'
        ),
    event_summary varchar2(500 char) not null
        annotations (
            display_label 'Event Summary',
            description 'Short business summary of the audit event.',
            value_required 'true',
            primary_display_column 'true',
            semantic_type 'description'
        ),
    old_value_text varchar2(1000 char)
        annotations (
            display_label 'Old Value Text',
            description 'Previous value or business state where the audit event reflects a change.',
            semantic_type 'description'
        ),
    new_value_text varchar2(1000 char)
        annotations (
            display_label 'New Value Text',
            description 'New value or business state where the audit event reflects a change.',
            semantic_type 'description'
        ),
    source_document_type varchar2(30 char)
        annotations (
            display_label 'Source Document Type',
            description 'Source business document type that caused the audit event where relevant.',
            semantic_type 'type'
        ),
    source_document_number varchar2(50 char)
        annotations (
            display_label 'Source Document Number',
            description 'Source business document number that caused the audit event where relevant.',
            semantic_type 'reference_number'
        ),
    event_at timestamp with time zone default systimestamp not null
        annotations (
    ...
    event_by varchar2(128 char) default sys_context('USERENV','SESSION_USER') not null
        annotations (
            display_label 'Event By',
            description 'Database user that recorded the audit event.',
            display_in_form 'false',
            semantic_type 'user_name'
        ),
    ...
    constraint ck_scm_audit_events_02 check (
        record_type_code in (
            'BUSINESS_PARTNER',
            'PARTNER_SITE',
            'WAREHOUSE',
    ...
    constraint ck_scm_audit_events_03 check (
        event_type_code in (
            'CREATED',
            'UPDATED',
            'STATUS_CHANGED',
    ...
    display_label 'Audit Events',
    description 'Stores generic audit history linked to supported warehouse records.',
    application_module 'Inventory and Warehouse Management'
);
